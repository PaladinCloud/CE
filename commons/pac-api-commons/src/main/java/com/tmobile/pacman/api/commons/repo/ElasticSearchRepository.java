...
//					requestBody.put(SORT, list);
					...
					if (fieldName != null) {
						...
						if(fieldName.equalsIgnoreCase("isAutofixPlanned.keyword")){
...
//						requestBody.put(SORT, list);
	...
	public Map<String, Object> getEnvAndTotalDistributionForIndexAndType(String index, String type,
		...
		String urlToQuery = buildAggsURL(esUrl, index, type);
		LOGGER.info("UrlToQuery {}"+urlToQuery);
		Map<String, Object> requestBody = new HashMap<String, Object>();
		Map<String, Object> matchFilters = Maps.newHashMap();
	 ...
	 * @throws Exception
	 */
	public List<Map<String, Object>> getCompliancePercentForPolicyPack(String index,String type, String requestJson) throws Exception{

		try {
			LOGGER.debug("inside ElasticSearchRepository.getCompliancePercentForPolicyPack");
			long docCount=getTotalDocCountWithConditions(index,type, requestJson.replace(", \"size\":"+ES_PAGE_SIZE+", \"sort\":{\"date\":{\"order\":\"asc\"}}",""));
			LOGGER.debug("inside ElasticSearchRepository.getCompliancePercentForPolicyPack, docCount is "+docCount);
			StringBuilder urlToQueryBuffer = new StringBuilder(esUrl).append(FORWARD_SLASH).append(index);
			if (!Strings.isNullOrEmpty(type)) {
				urlToQueryBuffer.append(FORWARD_SLASH).append(type);
			}
			urlToQueryBuffer.append(FORWARD_SLASH).append(_SEARCH);
			String urlToQuery = urlToQueryBuffer.toString();
			//return prepareResultsUsingScroll(0, docCount, urlToQuery, requestJson);
			return prepareResultsUsingPagination(index,requestJson,0,docCount);
		} catch (Exception e) {
			LOGGER.error("Exception inside getCompliancePercentForPolicyPack method of ElasticSearchRepository", e);
			throw e;
		}
	}
	...
	public Boolean saveActivityLogToES(String dataSource, Map<String, Object> data) throws Exception {
		try {
			...
			if(!ESUtils.isValidIndex(esUrl, dataSource)) {
				String url = new StringBuilder(esUrl).append("/").append(dataSource).toString();
				com.tmobile.pacman.commons.utils.CommonUtils.doHttpPut(url, activitylog_mappings);
			}
			StringBuilder esQueryUrl = new StringBuilder(esUrl);
	 ...
	 * @throws Exception
	 */
	public List<Map<String,Object>> fetchActivityLogs(String dataSource,String targetType, String request, String requestForCount, int size, int from, StringBuilder totalCount) throws Exception {
		try{
			LOGGER.info("inside ElasticSearchRepository:::fetchActivityLogs");
			long docCount=getTotalDocCountWithConditions(dataSource,"", requestForCount);
			totalCount.setLength(0);
			totalCount.append(String.valueOf(docCount));
			Long documentCount = size==0 ? docCount : size;
			LOGGER.info("inside ElasticSearchRepository:::fetchActivityLogs, docCount is "+docCount);
			StringBuilder urlToQueryBuffer = new StringBuilder(esUrl).append(FORWARD_SLASH).append(dataSource);
			if (!Strings.isNullOrEmpty(targetType)) {
				urlToQueryBuffer.append(FORWARD_SLASH).append(targetType);
			}
			urlToQueryBuffer.append(FORWARD_SLASH).append(_SEARCH);
			String urlToQuery = urlToQueryBuffer.toString();
			//return prepareResultsUsingScroll(from, documentCount, urlToQuery, request);
			return prepareResultsUsingPagination(dataSource,request,from,documentCount);
		} catch(Exception exception){
			LOGGER.error("Exception occurred in ElasticSearchRepository:::fetchActivityLogs");
			throw new Exception(exception.getMessage());
		}
	}
	...
	public Map<String, Object> getPolicyCompDetailsByAcctIdAndTags(String index, String requestBody) throws Exception {
		String urlToQuery = buildAggsURL(esUrl, index, null);
		Map<String, Object> policyCompDetailsMap = new HashMap<>();
		Gson gson = new GsonBuilder().create();
		try {
			LOGGER.debug("inside ElasticSearchRepository.getPolicyCompDetailsByAcctIdAndTags ");
			String responseDetails = PacHttpUtils.doHttpPost(urlToQuery, requestBody);
			JsonParser jsonParser = new JsonParser();
			JsonObject resultJson = (JsonObject) jsonParser.parse(responseDetails);
			Optional bucketsArrayOptional = Optional.ofNullable(resultJson).map(obj -> obj.getAsJsonObject("aggregations")).map(obj -> obj.getAsJsonObject("policyId")).map(obj->obj.getAsJsonArray("buckets"));
			if(bucketsArrayOptional.isPresent()) {
				JsonArray jsonArray = (JsonArray)bucketsArrayOptional.get();
				populatePolicyCompDetailsMap(jsonArray,policyCompDetailsMap,"");
			}
		} catch (Exception exception) {
			LOGGER.error("Exception occurred in ElasticSearchRepository.getPolicyCompDetailsByAcctIdAndTags");
			throw new Exception(exception.getMessage());
		}
		return policyCompDetailsMap;
	}

	/**
	 * populate map recursively
	 * @param jsonArray
	 * @param policyCompDetailsMap
	 */
	private void populatePolicyCompDetailsMap(JsonArray jsonArray, Map<String, Object> policyCompDetailsMap, String tagKey) {
		jsonArray.forEach(obj -> {
			JsonObject jsonObj = (JsonObject) obj;
			Iterator iterator = jsonObj.keySet().iterator();
			Map<String, Object> detailsMap = new HashMap<>();
			if(tagKey.startsWith("tags.")){
				policyCompDetailsMap.put(tagKey+"."+jsonObj.get("key").getAsString(), detailsMap);
			} else{
				policyCompDetailsMap.put(jsonObj.get("key").getAsString(), detailsMap);
			}
			int innerJsonObjectCount=0;
			while(iterator.hasNext()){
				String key = (String) iterator.next();
				if(jsonObj.get(key) instanceof JsonObject){
					innerJsonObjectCount++;
					JsonObject jsonobj = (JsonObject)jsonObj.get(key);
					Optional bucketsArrayOptional = Optional.ofNullable(jsonobj).map(obj1->obj1.getAsJsonArray("buckets"));
					if(bucketsArrayOptional.isPresent()) {
						JsonArray jarray = (JsonArray)bucketsArrayOptional.get();
						if(jarray.size()>0){
							populatePolicyCompDetailsMap(jarray,detailsMap,key);
						}
					}
				}
			}
			if(innerJsonObjectCount==0){
				policyCompDetailsMap.put(jsonObj.get("key").getAsString(),jsonObj.get("doc_count").getAsString());
			}
		});
	}
	 ...
	 * @return  countDetailsMap
	 *
	 */

	public Map<String,Object> getResourceCountByAcctIdAndTags(String assetGroup, String requestBodyJson) throws Exception {
		StringBuilder urlToQuery = new StringBuilder(esUrl).append(FORWARD_SLASH).append(assetGroup).append(FORWARD_SLASH).append(SEARCH);
		Map<String,Object> countDetailsMap = new HashMap<>();
		JsonParser jsonParser;
		JsonObject resultJson;
		Gson gson = new GsonBuilder().create();
		try {
			LOGGER.debug("inside ElasticSearchRepository.getResourceCountByAcctIdAndTags ");
			String responseDetails = PacHttpUtils.doHttpPost(urlToQuery.toString(), requestBodyJson);
			jsonParser = new JsonParser();
			resultJson = (JsonObject) jsonParser.parse(responseDetails);
			Optional bucketsArrayOptional = Optional.ofNullable(resultJson).map(obj -> obj.getAsJsonObject("aggregations")).map(obj -> obj.getAsJsonObject("targetType")).map(obj->obj.getAsJsonArray("buckets"));
			if(bucketsArrayOptional.isPresent()){
				JsonArray jsonArray = (JsonArray)bucketsArrayOptional.get();
				jsonArray.forEach(obj -> {
					JsonObject jsonObj = (JsonObject) obj;
					Iterator iterator = jsonObj.keySet().iterator();
					Map<String,Object> detailsMap = new HashMap<>();
					String tagType = jsonObj.get("key").toString();
					countDetailsMap.put(tagType,detailsMap);
					while(iterator.hasNext()){
						String key = (String) iterator.next();
						if("doc_count".equalsIgnoreCase(key) || "key".equalsIgnoreCase(key)){
							continue;
						} else if(jsonObj.get(key) instanceof JsonObject){
							JsonObject jsonObject=jsonObj.get(key).getAsJsonObject();
							addCountDetailsToMap(jsonObject, detailsMap, key);
						}
					}
				});
			}
		} catch (Exception e) {
			LOGGER.error("Exception occurred in ElasticSearchRepository.getResourceCountByAcctIdAndTags method");
			throw new Exception(e.getMessage());
		}
		return countDetailsMap;
	}

	private void addCountDetailsToMap(JsonObject jsonObject, Map<String,Object> detailsMap, String tagValue) {
		JsonArray jarray = jsonObject.getAsJsonArray("buckets");
		if(jarray.size()>0){
			jarray.forEach(obj -> {
				JsonObject jsonObj = (JsonObject) obj;
				if(tagValue.startsWith("tags.")){
					detailsMap.put(tagValue+"."+jsonObj.get("key").getAsString(), jsonObj.get("doc_count").getAsLong());
				} else{
					detailsMap.put(jsonObj.get("key").getAsString(), jsonObj.get("doc_count").getAsLong());
				}
			});
		}
	}
	...
	public Map<String,String> getAcctNamesByIds(String assetGroup, String requestBodyJson) throws Exception {
		LOGGER.info("inside getResourceCountByAcctIdAndTags ");
		Map<String, String> acctIdAcctNameMap = new HashMap<>();
		StringBuilder urlToQuery = new StringBuilder(esUrl).append(FORWARD_SLASH).append(assetGroup).append(FORWARD_SLASH).append(SEARCH);
		Map<String, Object> countDetailsMap = new HashMap<>();
		JsonParser jsonParser;
		JsonObject resultJson;
		...
		try {
			String responseDetails = PacHttpUtils.doHttpPost(urlToQuery.toString(), requestBodyJson);
			Map<String,Object> acctDetailsMap = gson.fromJson(responseDetails,Map.class);
			Optional acctIdBuckets = Optional.ofNullable(acctDetailsMap.get("aggregations")).map(obj -> ((Map<String,Object>)obj).get("acctId")).map(obj -> ((Map<String,Object>)obj).get("buckets"));
			if(acctIdBuckets.isPresent()){
				List<Map<String,Object>> bucketsList = (List<Map<String,Object>>)acctIdBuckets.get();
				for(Map<String,Object> map : bucketsList){
					String acctId = (String) map.get("key");
					Map<String,Object> acctNameMap = (Map<String,Object>)map.get("acctName");
					List<Map<String,Object>> acctNamesBuckets = (List<Map<String,Object>>)acctNameMap.get("buckets");
					acctNamesBuckets.stream().forEach(acctNameBucket -> {
						String acctName = (String) acctNameBucket.get("key");
						acctIdAcctNameMap.put(acctId,acctName);
					});
				}
			}
		} catch (Exception e) {
	...
	public List<Map<String, Object>> getDataFromES(String dataSource, String targetType, Map<String, Object> queryMap) throws Exception {
		...
		urlToQueryBuffer.append(FORWARD_SLASH).append(_SEARCH);
		String urlToQuery = urlToQueryBuffer.toString();
		//return prepareResultsUsingScroll(0, totalDocs, urlToQuery, requestBody);
