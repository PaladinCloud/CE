        ...
        if (propertySourcesArray.size() > 0) {
            for (int i = 0; i < propertySourcesArray.size(); i++) {
                ...
                if (propertySource.get(PacmanSdkConstants.NAME).toString().contains("application")) {
                    ...
                    appPropsHashtable = new Gson().fromJson(appProps, new TypeToken<Hashtable<String, Object>>() {
                    }.getType());
                }
                if (propertySource.get(PacmanSdkConstants.NAME).toString().contains("rule")) {
                    ...
                    rulePropsHashtable = new Gson().fromJson(ruleProps, new TypeToken<Hashtable<String, Object>>() {
                    }.getType());
                }
                if (propertySource.get(PacmanSdkConstants.NAME).toString().contains("batch")) {
                    ...
                    batchPropsHashtable = new Gson().fromJson(batchProps, new TypeToken<Hashtable<String, Object>>() {
                    }.getType());
                }
            }
        } else {
