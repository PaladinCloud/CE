...
public class NetworkInventoryCollector {
    ...
    public List<NetworkVH> fetchNetworkInventory(String project) throws Exception {
        List<NetworkVH> networkVHList=new ArrayList<>();
        Iterable<Network> networkList = gcpCredentialsProvider.getNetworksClient().list(project).iterateAll();
        for (Network network:networkList) {
            NetworkVH networkVH=new NetworkVH();
            networkVH.setId(String.valueOf(network.getId()));
            networkVH.setProjectName(project);
