...
@Component
public class SQLDatabaseInventoryCollector implements Collector {

    private static final Logger log = LoggerFactory.getLogger(SQLDatabaseInventoryCollector.class);
    @Autowired
    ...
    public List<SQLDatabaseVH> collect(SubscriptionVH subscription,
                                       Map<String, Map<String, String>> tagMap) {
        List<SQLDatabaseVH> sqlDatabaseList = new ArrayList<>();
        ...
        for (SqlServer sqlServer : sqlServers) {
            ...
            for (SqlDatabase sqlDatabase : sqlDatabases) {
                if (!sqlDatabase.name().contentEquals("master")) {
                    ...
                    sqlDatabaseVH.setRegion(Util.getRegionValue(subscription, sqlDatabase.regionName()));
        ...
        log.info("Target Type : {}  Total: {} ", "Sql Database", sqlDatabaseList.size());
