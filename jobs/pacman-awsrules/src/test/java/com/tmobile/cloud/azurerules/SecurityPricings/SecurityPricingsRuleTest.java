...
public class SecurityPricingsRuleTest {
    ...
    public void executeTest() throws Exception {
        ...
        assertThat(securityPricingRule.execute(getMapNotExistString("r_123 "), getMapNotExistString("r_123 ")),
                is(notNullValue()));

        when(PacmanUtils.getPacmanHost(anyString())).thenReturn("host");

        when(PacmanUtils.checkISResourceIdExistsFromElasticSearch("_resourceid", "azure_securitypricings/_search", "", ""))
                .thenReturn(true);
        when(PacmanUtils.doesAllHaveValue(anyString(), anyString(), anyString())).thenReturn(false);
        assertThatThrownBy(
                () -> securityPricingRule.execute(getMapString("r_123 "), getMapString("r_123 ")))
