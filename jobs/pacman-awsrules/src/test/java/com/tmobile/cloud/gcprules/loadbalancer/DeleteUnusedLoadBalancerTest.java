...
public class DeleteUnusedLoadBalancerTest {

    @InjectMocks
    DeleteUnusedLoadBalancer deleteUnusedLoadBalancer;
    ...
    public void executeSuccessTest() throws Exception {
        ...
        when(GCPUtils.getHitsArrayFromEs(anyObject(), anyObject())).thenReturn(getHitsJsonArrayForUnusedLoadBalancer());

        when(Annotation.buildAnnotation(anyObject(), anyObject())).thenReturn(CommonTestUtils.getMockAnnotation());
        when(PacmanUtils.doesAllHaveValue(anyString(), anyString(), anyString())).thenReturn(
                true);
        assertThat(deleteUnusedLoadBalancer.execute(getMapString("r_123 "), getMapString("r_123 ")).getStatus(),
                is(PacmanSdkConstants.STATUS_SUCCESS));

    }
    private JsonArray getHitsJsonArrayForUnusedLoadBalancer(){
        Gson gson = new Gson();
    ...
    public void executeFailureTest() throws Exception {
        ...
        when(GCPUtils.getHitsArrayFromEs(anyObject(), anyObject()))
                .thenReturn(getFailureHitsJsonArrayForUnusedLoadBalancer());
    ...
    private JsonArray getFailureHitsJsonArrayForUnusedLoadBalancer() {
        Gson gson = new Gson();
        JsonObject jsonObject = new JsonObject();
        jsonObject.add("_source", gson.fromJson(
                "{\n" +
    ...
    public void executeFailureTestWithInvalidInputException() throws Exception {
        ...
        when(PacmanUtils.getPacmanHost(anyString())).thenReturn("host");
        when(GCPUtils.getHitsArrayFromEs(anyObject(), anyObject())).thenReturn(getFailureHitsJsonArrayForUnusedLoadBalancer());

        when(Annotation.buildAnnotation(anyObject(), anyObject())).thenReturn(CommonTestUtils.getMockAnnotation());
        when(PacmanUtils.doesAllHaveValue(anyString(), anyString(), anyString())).thenReturn(
                false);
        assertThatThrownBy(() -> deleteUnusedLoadBalancer.execute(getMapString("r_123 "), getMapString("r_123 "))).isInstanceOf(InvalidInputException.class);
    }
    ...
    public void getHelpTextTest() {
        assertThat(deleteUnusedLoadBalancer.getHelpText(), is(notNullValue()));
    }
}
