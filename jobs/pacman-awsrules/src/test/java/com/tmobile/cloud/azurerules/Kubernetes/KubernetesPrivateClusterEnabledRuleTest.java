...
public class KubernetesPrivateClusterEnabledRuleTest {

    @InjectMocks
    KubernetesPrivateClusterEnableRule kubernetesPrivateClusterEnableRule;
    ...
    public void executeSucessTest() throws Exception {
        ...
        when(RulesElasticSearchRepositoryUtil.getQueryDetailsFromES(anyString(),anyObject(),
                anyObject(),
                anyObject(), anyObject(), anyInt(), anyObject(), anyObject(), anyObject())).thenReturn(getHitJsonArrayForPrivateCluster());
        assertThat(kubernetesPrivateClusterEnableRule.execute(CommonTestUtils.getMapString("r_123 "),
                CommonTestUtils.getMapString("r_123 ")).getStatus(), is(PacmanSdkConstants.STATUS_SUCCESS));
    }
    private JsonObject getHitJsonArrayForPrivateCluster(){
        Gson gson=new Gson();
        ...
        jsonObject.add("hits", gson.fromJson("{\n    \"hits\": [\n   {\n" +
                ...
                "            \"privateFQDN\": \"privatecluster-dns-c15b4cc3.50cec619-47a9-4fef-b16f-c85adb37333c.privatelink.eastus.azmk8s.io\",\n" +
                "            \"autoScalerProfile\": {\n" +
    ...
    public void executeFailureTest() throws Exception {
        ...
        when(RulesElasticSearchRepositoryUtil.getQueryDetailsFromES(anyString(),anyObject(),
                anyObject(),
                anyObject(), anyObject(), anyInt(), anyObject(), anyObject(), anyObject())).thenReturn(getFailureJsonArrayForPrivateCluster());
        assertThat(kubernetesPrivateClusterEnableRule.execute(CommonTestUtils.getMapString("r_123 "),
                CommonTestUtils.getMapString("r_123 ")).getStatus(), is(PacmanSdkConstants.STATUS_FAILURE));
    }

    private JsonObject getFailureJsonArrayForPrivateCluster(){
        Gson gson=new Gson();
        JsonObject jsonObject = new JsonObject();
        jsonObject.add("hits", gson.fromJson("{\n    \"hits\": [\n{\n" +
                "        \"_index\": \"azure_kubernetes\",\n" +
    ...
    public void getHelpTextTest() {
        assertThat(kubernetesPrivateClusterEnableRule.getHelpText(),is(notNullValue()));
    }
}
