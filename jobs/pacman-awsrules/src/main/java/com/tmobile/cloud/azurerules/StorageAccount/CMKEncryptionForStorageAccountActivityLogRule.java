...
@PacmanPolicy(key = "check-for-storageAcc-activitylog-encryption", desc = "Azure activity log storage container is encrypted with BYOK (Bring Your Own Key) ", severity = PacmanSdkConstants.SEV_MEDIUM, category = PacmanSdkConstants.SECURITY)
public class CMKEncryptionForStorageAccountActivityLogRule extends BasePolicy {
    private static final Logger logger = LoggerFactory.getLogger(CMKEncryptionForStorageAccountActivityLogRule.class);
    ...
    public PolicyResult execute(Map<String, String> ruleParam, Map<String, String> resourceAttributes) {
        ...
        if (!StringUtils.isNullOrEmpty(resourceId)) {
            ...
            try {
                isValid = checkCMKEncryption(esUrl, mustFilter);
                if (!isValid) {
    ...
    private boolean checkCMKEncryption(String esUrl, Map<String, Object> mustFilter) throws Exception {
        ...
        if (resultJson != null && resultJson.has(PacmanRuleConstants.HITS)) {
            ...
            if (hitsJsonArray.size() > 0) {
                ...
                JsonArray storageAccountLogList=jsonDataItem.getAsJsonObject().get("storageAccountLogList").getAsJsonArray();

                if(!storageAccountLogList.isEmpty()){
                    for(int i=0;i<storageAccountLogList.size();i++){
