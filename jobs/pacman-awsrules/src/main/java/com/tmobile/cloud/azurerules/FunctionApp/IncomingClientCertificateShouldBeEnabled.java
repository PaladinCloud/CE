...
@PacmanPolicy(key = "check-for-functionapp-incoming-certificate-enabled", desc = "checks if incoming certificate is enabled", severity = PacmanSdkConstants.SEV_MEDIUM, category = PacmanSdkConstants.SECURITY)
public class IncomingClientCertificateShouldBeEnabled extends BasePolicy {
    private static final Logger logger = LoggerFactory.getLogger(IncomingClientCertificateShouldBeEnabled.class);
    @Override
    public PolicyResult execute(Map<String, String> ruleParam, Map<String, String> resourceAttributes) {
        ...
        if (!StringUtils.isNullOrEmpty(url)) {
            esUrl = url + "/azure_functionapp/_search";
        }
        ...
        if (!StringUtils.isNullOrEmpty(resourceId)) {
            ...
            try {
                isValid = checkFunctionAppIncomingCertificate(esUrl, mustFilter);
                if (!isValid) {
    ...
    private boolean checkFunctionAppIncomingCertificate(String esUrl, Map<String, Object> mustFilter) throws Exception {
        ...
        if (resultJson != null && resultJson.has(PacmanRuleConstants.HITS)) {
            ...
            if (hitsJsonArray.size() > 0) {
                ...
                boolean clientCertificateEnabled = jsonDataItem.get(PacmanRuleConstants.CLIENT_CERT_ENABLED).getAsBoolean();
    ...
    public String getHelpText() {
        return "This rule checks if incoming client certificate is enabled";
    }
}
