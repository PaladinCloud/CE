...
import java.util.*;
@PacmanPolicy(key = "enable-rbac-role-for-kubernetes", desc = "Ensure 'All users with following rules' is set to owner in security alert emails", severity = PacmanSdkConstants.SEV_MEDIUM, category = PacmanSdkConstants.SECURITY)
public class KubernetesRbacRule extends BasePolicy {
    private static final Logger logger = LoggerFactory.getLogger(com.tmobile.cloud.azurerules.Kubernetes.KubernetesRbacRule.class);
    @Override
    public PolicyResult execute(Map<String, String> ruleParam, Map<String, String> resourceAttributes) {
        logger.info("Executing KubernetesRbacRule..");
        ...
        try {
            isValid = checkRbacEnabledForKubernetes(esUrl, mustFilter);
            if (!isValid) {
    ...
    private boolean checkRbacEnabledForKubernetes(String esUrl, Map<String, Object> mustFilter) throws Exception {
        ...
        if (resultJson != null && resultJson.has(PacmanRuleConstants.HITS)) {
            ...
            for (int i=0;i<hitsJsonArray.size();i++) {
                ...
                Boolean enableRbac = jsonDataItem.get("enableRBAC").getAsBoolean();
                if(Boolean.TRUE.equals(enableRbac)){
    ...
    public String getHelpText() {
        return "Enable RBAC for Azure Kubernetes.";
    }
}
