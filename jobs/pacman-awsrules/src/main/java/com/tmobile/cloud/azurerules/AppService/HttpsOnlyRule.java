...
@PacmanPolicy(key = "check-for-https-only-rule", desc = "enable https only", severity = PacmanSdkConstants.SEV_MEDIUM, category = PacmanSdkConstants.SECURITY)
public class HttpsOnlyRule extends BasePolicy {
    ...
    public PolicyResult execute(Map<String, String> ruleParam, Map<String, String> resourceAttributes) {
        ...
        if (!StringUtils.isNullOrEmpty(resourceId)) {
            ...
            try {
                isValid = isHttpsonlyEnabled(esUrl, mustFilter);
            } catch (Exception e) {
            ...
            if (!isValid) {
                ...
                annotation.put(PacmanSdkConstants.DESCRIPTION,
                        "Ensure that  Azure App Service web applications redirect all non-secure HTTP traffic to HTTPS in order to encrypt the communication between applications and web clients");
    ...
    private boolean isHttpsonlyEnabled(String esUrl, Map<String, Object> mustFilter) throws Exception {
        ...
        if (resultJson != null && resultJson.has(PacmanRuleConstants.HITS)) {
            ...
            if (hitsJsonArray.size() > 0) {
                ...
                Boolean isHttp20Enabled = jsonDataItem.getAsJsonObject()
                        .get(PacmanRuleConstants.HTTPS_ONLY).getAsBoolean();
    ...
    public String getHelpText() {
        return "check https was enabled for webapp";
    }
}
