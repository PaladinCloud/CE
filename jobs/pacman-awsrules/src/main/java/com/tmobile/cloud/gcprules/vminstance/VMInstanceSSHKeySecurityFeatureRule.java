...
import java.util.*;
@PacmanPolicy(key = "check-for-block-project-ssh-key", desc = "checks if instance-level SSH keys is enabled", severity = PacmanSdkConstants.SEV_MEDIUM, category = PacmanSdkConstants.SECURITY)
public class VMInstanceSSHKeySecurityFeatureRule extends BasePolicy {

    private static final Logger logger = LoggerFactory.getLogger(VMInstanceSSHKeySecurityFeatureRule.class);

    private static final String keyValue="block-project-ssh-keys";

    @Override
    public PolicyResult execute(Map<String, String> ruleParam, Map<String, String> resourceAttributes) {
        logger.debug("========VMInstanceSSHKeySecurityFeatureRule started=========");
        ...
        if (!StringUtils.isNullOrEmpty(resourceId)) {
            ...
            try {
                ...
                if (!isBlockProjectWideSSHEnable) {
                    ...
                    annotation.put(PacmanSdkConstants.DESCRIPTION, "Block Project-Wide SSH Keys is not enabled");
                    annotation.put(PacmanRuleConstants.SEVERITY, severity);
                    annotation.put(PacmanRuleConstants.CATEGORY, category);

                    issue.put(PacmanRuleConstants.VIOLATION_REASON, "Block Project-Wide SSH Keys is not enabled");
                    issueList.add(issue);
                    annotation.put("issueDetails", issueList.toString());
                    logger.debug("========VMInstanceSSHKeySecurityFeatureRule ended with an annotation {} : =========", annotation);
    ...
    public String getHelpText() {
        return "This rule checks if Google Compute Engine instances are configured to ignore GCP project-wide (shared) public SSH keys and use instance-level SSH keys instead.";
    }
