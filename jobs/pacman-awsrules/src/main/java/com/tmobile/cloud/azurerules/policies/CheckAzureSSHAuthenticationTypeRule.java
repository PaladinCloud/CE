...
@PacmanPolicy(key = "check-for-ssh-authentication-type", desc = "Azure policy for checking azure virtual machines are configured to use SSH keys instead of username/password credentials for SSH authentication", severity = PacmanSdkConstants.SEV_HIGH, category = PacmanSdkConstants.SECURITY)
public class CheckAzureSSHAuthenticationTypeRule extends BasePolicy {

    private static final Logger logger = LoggerFactory.getLogger(CheckAzureSSHAuthenticationTypeRule.class);
