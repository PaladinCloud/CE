...
@PacmanPolicy(key = "check-for-unrestricted-sql-database-access", desc = "Azure policy for checking SQL servers allow unrestricted inbound access for all sql databases hosted on server", severity = PacmanSdkConstants.SEV_HIGH, category = PacmanSdkConstants.SECURITY)
public class CheckAzureUnrestrictedSqlDatabaseAccessRule extends BasePolicy {
