...
public class PrincipleOfLeastPrivilegeRule extends BaseRule {
    ...
    private boolean checkIfServiceAccountHasLeastPrivilege(String vmEsURL, Map<String, Object> mustFilter) throws Exception {
        ...
        if (hitsJsonArray.size() > 0) {
            ...
            JsonObject rolesMembers = serviceAccountObject.getAsJsonObject()
                    .get(PacmanRuleConstants.ROLES_MEMBERS).getAsJsonObject();
          JsonArray editorMembers=  rolesMembers.getAsJsonArray("roles/editor");
          JsonArray ownerMembers=rolesMembers.getAsJsonArray("roles/owner");
          JsonArray adminMembers=rolesMembers.getAsJsonArray("roles/admin");
          for(JsonElement editorMember:editorMembers)
          {
              if (editorMember.getAsString().startsWith("serviceAccount"))
                  return false;
          }
            for(JsonElement ownerMember:ownerMembers)
            {
                if (ownerMember.getAsString().startsWith("serviceAccount"))
                    return false;
            }
            for(JsonElement adminMember:adminMembers)
            {
                if (adminMember.getAsString().startsWith("serviceAccount"))
                    return false;
            }
