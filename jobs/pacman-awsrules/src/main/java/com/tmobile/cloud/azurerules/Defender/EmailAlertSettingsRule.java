...
@PacmanPolicy(key = "set-all-users-to-owner-email-notification", desc = "Ensure 'All users with following rules' is set to owner in security alert emails", severity = PacmanSdkConstants.SEV_MEDIUM, category = PacmanSdkConstants.SECURITY)
public class EmailAlertSettingsRule extends BasePolicy {

    private static final Logger logger = LoggerFactory.getLogger(com.tmobile.cloud.azurerules.Defender.EmailAlertSettingsRule.class);
    @Override
    public PolicyResult execute(Map<String, String> ruleParam, Map<String, String> resourceAttributes) {
        ...
        try {
            isValid = checkAllUsersSetToOwner(esUrl, mustFilter);
            if (!isValid) {
    ...
    private boolean checkAllUsersSetToOwner(String esUrl, Map<String, Object> mustFilter) throws Exception {
        ...
        if (resultJson != null && resultJson.has(PacmanRuleConstants.HITS)) {
            ...
            for (int i=0;i<hitsJsonArray.size();i++) {
                ...
                JsonArray rolesJsonArray= jsonDataItem.getAsJsonObject("properties").getAsJsonObject("notificationsByRole").getAsJsonArray("roles");
                for(int j=0;j<rolesJsonArray.size();j++){
                    String roleStr = rolesJsonArray.get(j).getAsString();
    ...
    public String getHelpText() {
        return "Email notification policy for Azure-MS defender";
    }
}
