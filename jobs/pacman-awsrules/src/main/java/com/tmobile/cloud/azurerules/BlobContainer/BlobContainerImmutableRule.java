...
@PacmanPolicy(key = "check-if-blob-container-is-immutable", desc = "azure blob container should be immutable", severity = PacmanSdkConstants.SEV_HIGH, category = PacmanSdkConstants.SECURITY)
public class BlobContainerImmutableRule extends BasePolicy {
    private static final Logger logger = LoggerFactory.getLogger(BlobContainerImmutableRule.class);

    @Override
    public PolicyResult execute(Map<String, String> ruleParam, Map<String, String> resourceAttributes) {
        logger.info("Executing Azure blob container immutability rule");
        ...
        if (!StringUtils.isNullOrEmpty(url)) {
            esUrl = url + "/azure_blobcontainer/_search";
        }
        ...
        if (!StringUtils.isNullOrEmpty(resourceId)) {
            ...
            try {
                isValid = checkBlobContainerImmutability(esUrl, mustFilter);
                if (!isValid) {
    ...
    private boolean checkBlobContainerImmutability(String esUrl, Map<String, Object> mustFilter) throws Exception {
    ...
    public String getHelpText() {
        return "This rule verifies whether blob container is immutable";
    }
}
