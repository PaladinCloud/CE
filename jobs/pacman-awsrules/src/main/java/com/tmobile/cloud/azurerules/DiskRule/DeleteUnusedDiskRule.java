...
@PacmanPolicy(key = "check-for-unused-disk", desc = "Delete any unattached (unused) Microsoft Azure virtual machine disk volumes available within your Azure cloud account", severity = PacmanSdkConstants.SEV_MEDIUM, category = PacmanSdkConstants.SECURITY)
public class DeleteUnusedDiskRule extends BasePolicy {
    private static final Logger logger = LoggerFactory
            .getLogger(DeleteUnusedDiskRule.class);
    ...
    public PolicyResult execute(Map<String, String> ruleParam, Map<String, String> resourceAttributes) {
        ...
        if (!StringUtils.isNullOrEmpty(resourceId)) {
            ...
            try {
                isValid = checkForEmptyVmDisk(esUrl, mustFilter);
            ...
            if (isValid) {
                ...
                logger.debug(
                        "Azure VM disk is unused and can be safely deleted.");
                return new PolicyResult(PacmanSdkConstants.STATUS_FAILURE, PacmanRuleConstants.FAILURE_MESSAGE,
                        annotation);
            }
        }

        logger.debug("The selected disk volume is attached to a Microsoft Azure virtual machine");
        return new PolicyResult(PacmanSdkConstants.STATUS_SUCCESS, PacmanRuleConstants.SUCCESS_MESSAGE);

    }
    private boolean checkForEmptyVmDisk(String esUrl, Map<String, Object> mustFilter) throws Exception {
        ...
        if (resultJson != null && resultJson.has(PacmanRuleConstants.HITS)) {
            ...
            if (hitsJsonArray.size() > 0) {
                ...
                if (diskInner != null) {
                        String diskState = diskInner.getAsJsonObject()
                                .get("properties.diskState").getAsString();

                        if (diskState.equalsIgnoreCase("Unattached")) {
    ...
    public String getHelpText() {
        return "Check for any unattached (unused) Microsoft Azure virtual machine disk volumes available within your Azure cloud account.";
    }
