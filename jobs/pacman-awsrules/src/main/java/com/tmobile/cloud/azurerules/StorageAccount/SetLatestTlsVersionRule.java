...
@PacmanPolicy(key = "check-for-latest-Tls-version-rule", desc = "Check storage Account is using the latest version of TLS encryption", severity = PacmanSdkConstants.SEV_MEDIUM, category = PacmanSdkConstants.SECURITY)
public class SetLatestTlsVersionRule extends BasePolicy {
    ...
    private boolean minTlsVersion(String esUrl, Map<String, Object> mustFilter,String tlsVersion) throws Exception {
        ...
        if (resultJson != null && resultJson.has(PacmanRuleConstants.HITS)) {
            ...
            if (hitsJsonArray.size() > 0) {
                ...
                String minTLSVersion=jsonDataItem.getAsJsonObject().get(PacmanRuleConstants.MIN_TLS_VERSION).isJsonNull()?null:jsonDataItem.getAsJsonObject().get(PacmanRuleConstants.MIN_TLS_VERSION).getAsString();
                logger.info("minTLSVersion",minTLSVersion);

                if(tlsVersion.equalsIgnoreCase(minTLSVersion)){
