...
@PacmanPolicy(key = "check-for-vpc-network-firewall-outbound-security", desc = "checks for vpc network public IP address", severity = PacmanSdkConstants.SEV_HIGH, category = PacmanSdkConstants.SECURITY)
public class VPCNetworkOutboundRule extends BasePolicy {

    private static final Logger logger = LoggerFactory.getLogger(VPCNetworkOutboundRule.class);
    ...
    public PolicyResult execute(Map<String, String> ruleParam, Map<String, String> resourceAttributes) {
        ...
        logger.debug("========VPCWithPublicIPAccess ended=========");
    ...
    private boolean verifyPorts(String vmEsURL, Map<String, Object> mustFilter, String[] ports) throws Exception {
        ...
        if (hitsJsonArray.size() > 0) {
            ...
            JsonArray destinationRanges = vpcFirewall.getAsJsonObject().get(PacmanRuleConstants.DESTINATIONRANGE)
                    .getAsJsonArray();
            JsonArray allow = vpcFirewall.getAsJsonObject().get(PacmanRuleConstants.ALLOW.toLowerCase())
                    .getAsJsonArray();

            if (!isDisabled && direction.equalsIgnoreCase(PacmanRuleConstants.EGRESS)
                    && validateSourceRanges(destinationRanges)) {
    ...
    boolean validateSourceRanges(JsonArray destinationRanges) {
        logger.debug("*********validate source Ranges {}", destinationRanges);

        for (JsonElement jsonElement : destinationRanges) {
            String destinationrange = jsonElement.toString().replaceAll("^\"|\"$", "");

            if (destinationrange.equals(PacmanRuleConstants.SOURCERANGE)) {
    ...
    public String getHelpText() {
        return "check public Access to VPC Fire wall outbound";
    }
