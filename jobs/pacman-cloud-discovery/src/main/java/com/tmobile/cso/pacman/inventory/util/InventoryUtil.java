...
import static com.tmobile.cso.pacman.inventory.util.InventoryConstants.REGION_GLOBAL;
...
public class InventoryUtil {
     ...
     * The delimiter.
     */
    private static final String delimiter = FileGenerator.DELIMITER;

    /**
     * The asg max record.
     */
    private static final int asgMaxRecord = 100;
     ...
     * @return the map
     */
    public static Map<String, List<NetworkInterface>> fetchNetworkInterfaces(BasicSessionCredentials temporaryCredentials, String skipRegions, String accountId, String accountName) {
            ...
            try {
                if (!skipRegions.contains(region.getName())) {
                    ...
                    if (!elbListTemp.isEmpty()) {
                        List<String> elbNames = elbListTemp.stream().map(elb -> {
                            return elb.getLoadBalancerName();
                        }).collect(Collectors.toList());
        ...
        s3buckets.parallelStream().forEach(bucket -> {
            ...
            try {
                ...
                if (null != policy && !StringUtils.isEmpty(policy.getPolicyText()))
                    bucketPolicy = policy.getPolicyText();
            ...
            try {
                if (!skipRegions.contains(region.getName())) {
                    ...
                    tmpVpcList.stream().forEach(vpc -> {
                        ...
                        vpcVH.setVpc(vpc);
                        List<VpcEndpoint> vpcEndPoints = ec2Client.describeVpcEndpoints(new DescribeVpcEndpointsRequest().withFilters(new Filter("vpc-id", Collections.singletonList(vpc.getVpcId())))).getVpcEndpoints();
            ...
            try {
                if (!skipRegions.contains(region.getName())) {
                    ...
                    for (String queueUrl : sqsUrls) {
                        try {
                            SQS sqsObj = objectMapper.convertValue(amazonSQS.getQueueAttributes(new GetQueueAttributesRequest(queueUrl, Collections.singletonList("All"))).getAttributes(),
                                    SQS.class);
