...
export class VulnReportTablesComponent implements OnDestroy, OnChanges {
    ...
    getPerformersData() {
        ...
        const performersQueryParam = {
            ag: this.selectedAssetGroup,
            severity: this.currentFilters,
            type: this.performersArr[this.performersCnt],
        };
        const performersTableUrl = environment.performersTable.url;
        const performersTableMethod = environment.performersTable.method;
        this.performersTableSubscription = this.commonResponseService
            .getData(
                performersTableUrl,
                performersTableMethod,
                performersPayload,
                performersQueryParam,
            )
            .subscribe(
                (response) => {
                    try {
    ...
    getRemediateData() {
        ...
        this.remediationTableSubscription = this.commonResponseService
            .getData(
                remediationTableUrl,
                remediationTableMethod,
                remediationPayload,
                remediationQueryParam,
            )
            .subscribe(
                (response) => {
                    try {
