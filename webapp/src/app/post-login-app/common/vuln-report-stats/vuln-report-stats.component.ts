...
export class VulnReportStatsComponent implements OnDestroy, OnChanges {
    ...
    getData() {
        ...
        this.statsSubscription = this.commonResponseService
            .getData(statsTableUrl, statsTableMethod, statsPayload, statsQueryParam)
            .subscribe(
                (response) => {
                    try {
      ...
      for ( let i = 0; i < this.statsData['severityinfo'].length ; i++ ) {
        for ( let j = 0; j < filterArr.length ; j++ ) {
          if (this.statsData['severityinfo'][i].severitylevel.toString() === filterArr[j]) {
            ...
            this.summaryObj.vulnerabilities =
                this.statsData['uniqueVulnCount'] || this.summaryObj.vulnerabilities;
            this.summaryObj.assetsAffected =
                this.statsData['assetsWithVulns'] || this.summaryObj.assetsAffected;
            this.summaryObj.occurrences =
                this.statsData['vulnerabilities'] || this.summaryObj.occurrences;
        } catch (error) {
