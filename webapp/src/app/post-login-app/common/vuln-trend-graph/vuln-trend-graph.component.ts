...
export class VulnTrendGraphComponent implements OnChanges, OnInit, OnDestroy {
        ...
        try {
            ...
            if (DataChange && DataChange.currentValue) {
                ...
                this.agList = [
                    ...
                    { text: this.agName, id: this.selectedAssetGroup },
                ];
    ...
    plotGraph(data) {
        ...
        const margin = { top: 45, right: 20, bottom: 20, left: 70 },
            width = parseInt(this.parentWidth, 10) - margin.left - margin.right,
        ...
        y0.domain([
            d3Array.min(newData, function (d) {
                return Math.min(d['new'], d['open'], 5);
            }),
            d3Array.max(newData, function (d) {
                return Math.max(d['new'], d['open'], 5);
            }),
        ]);
        ...
        svg.append('g')
                ...
                make_x_gridlines()
                    ...
                    .tickValues(
                        x.domain().filter(function (d, i) {
                            return !(i % (graphThis.months[graphThis.selectedMonth] * 4));
                        }),
                    ),
            )
            .selectAll('text')
            ...
            .attr('dy', '1.3em')
            .text(function (d, i) {
                return newData[i * (graphThis.months[graphThis.selectedMonth] * 4)].dateStr;
            });
        ...
        svg.append('g')
            ...
            .style('transform', function (d, i) {
                return i === 0 ? 'translate(2px, -24px)' : 'translate(-18px, -24px)';
            });
        ...
        svg.append('g')
            ...
            .style('fill', '#60727f')
            .style('transform', function (d, i) {
                return i === 0 ? 'translate(19px, -8px)' : 'translate(0px, -8px)';
            })
            .attr('x', function (d, i) {
                return x(d['date']);
            })
        ...
        svg.append('g')
            ...
            .style('transform', function (d, i) {
                return i === 0 ? 'translate(2px, -16px)' : 'translate(-18px, -16px)';
            });
        ...
        svg.append('g')
            ...
            .style('fill', '#ed0004')
            .style('transform', function (d, i) {
                return i === 0 ? 'translateX(19px)' : 'translateX(0px)';
            })
            .style('text-anchor', 'middle')
            .attr('x', function (d, i) {
                return x(d['date']);
    ...
    findMaxVal(obj) {
        try {
            if (!isNaN(obj.open) && !isNaN(obj.new)) {
                this.maxVal[0] = Math.max(this.maxVal[0], obj['open'], obj['new']);
                for (let i = 5; i > 0; i--) {
        ...
        try {
            if (this.adminAccess && this.notesResponse > 0) {
                ...
                this.recentData.push({
                    date: this.graphData[index].dateStr,
                    data: this.graphData[index].open,
                    selectedDate: true,
                });
                if (index !== this.graphData.length - 1) {
                    this.recentData.push({
                        date: this.graphData[index + 1].dateStr,
                        ...
                        if (this.notesData[i].data.length === 1) {
                            ...
                            if (data.dateStr === this.notesData[i].dateStr) {
                                ...
                                if (this.notesData[i].data[0].type === 'Global') {
                                    ...
                                    this.activeScope = [
                                        { id: this.selectedAssetGroup, text: this.agName },
                                    ];
                                }
                            }
                        } else {
                            ...
                            if (from === 'datepicker') {
                                for (let j = 0; j < this.notesData[i].data.length; j++) {
                                    if (
                                        (this.activeScope[0].id === 'global' &&
                                            this.notesData[i].data[j].type === 'Global') ||
                                        (this.activeScope[0].id !== 'global' &&
                                            this.notesData[i].data[j].type !== 'Global')
                                    ) {
    ...
    selectAg(tag) {
        ...
        for (let i = 0; i < this.notesData.length; i++) {
            if (
                this.dateSelected.toISOString().slice(0, 10) ===
                this.notesData[i].date.toISOString().slice(0, 10)
            ) {
                if (this.notesData[i].data.length === 1) {
                    if (
                        (this.notesData[i].data[0].type === 'Global' &&
                            this.activeScope[0].id === 'global') ||
                        (this.notesData[i].data[0].type === 'AssetGroup' &&
                            this.activeScope[0].id !== 'global')
                    ) {
                        this.notesText = this.notesData[i].data[0].note;
                        this.editMode = true;
                    ...
                    for (let j = 0; j < this.notesData[i].data.length; j++) {
                        if (
                            (this.activeScope[0].id === 'global' &&
                                this.notesData[i].data[j].type === 'Global') ||
                            (this.activeScope[0].id !== 'global' &&
                                this.notesData[i].data[j].type === 'AssetGroup')
                        ) {
                            this.notesText = this.notesData[i].data[j].note;
                            this.editMode = true;
                            newNote = false;
            ...
            if (this.adminAccess && this.notesResponse > 0) {
                ...
                for (let i = 0; i < this.graphData.length; i++) {
                    ...
                    if (this.graphData[i].dateStr.replace('/', '-') === data.date.slice(-5)) {
                        this.recentData.push({
                            date: this.graphData[i].dateStr,
                            data: this.graphData[i].open,
                            selectedDate: true,
                        });
                        if (i !== this.graphData.length - 1) {
                            this.recentData.push({
                                date: this.graphData[i + 1].dateStr,
    ...
    getDateData(date) {
        try {
            for (let i = 0; i < this.graphData.length; i++) {
                if (
                    date.getDate() === this.graphData[i].date.getDate() &&
                    date.getMonth() === this.graphData[i].date.getMonth()
                ) {
    ...
    updateNote() {
        try {
            if (this.notesText.trim() && this.updateNoteState !== 0) {
                ...
                this.updateNoteSubscription = this.commonResponseService
                    .getData(updateNoteUrl, updateNoteMethod, notesPayload, updateNoteParam)
                    .subscribe(
                        (response) => {
                            try {
                                this.openNotesModal = false;
    ...
    deleteNote() {
        try {
            if (this.updateNoteState !== 0) {
                ...
                this.deleteNoteSubscription = this.commonResponseService
                    .getData(deleteNoteUrl, deleteNoteMethod)
                    .subscribe(
                        (response) => {
                            try {
                                this.openNotesModal = false;
