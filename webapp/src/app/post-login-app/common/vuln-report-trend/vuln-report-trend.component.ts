...
export class VulnReportTrendComponent implements OnDestroy, OnChanges, AfterViewInit {
    ...
    ngOnChanges(changes: SimpleChanges) {
        try {
            ...
            if (DataChange && DataChange.currentValue) {
                ...
                if (
                    JSON.stringify(DataChange.currentValue) !== JSON.stringify(prev) &&
    ...
    getData() {
        ...
        try {
            ...
            this.vulnReportSubscription = this.commonResponseService
                .getData(vulnReportGraphUrl, vulnReportGraphMethod, graphPayload, graphQueryParam)
                .subscribe(
                    (response) => {
                        try {
    ...
    fetchNotesData() {
        try {
            ...
            this.vulnNotesSubscription = this.commonResponseService
                .getData(vulnReportNotesUrl, vulnReportNotesMethod, notesPayload, notesQueryParam)
                .subscribe(
                    (response) => {
                        try {
    ...
    processNotes(notes) {
        ...
        for (let p = 0; p < notesArr.length; p++) {
            ...
            notesObj[notesArr[p].date] = {
                data: [],
                date: notesArr[p].date,
                dateStr: notesArr[p].date.substring(0, 5),
            };
        }
    ...
    setAgNameFromSelectedAssetGroup(selectedAssetGroup) {
        this.assetTileService.getAssetGroupDisplayName(selectedAssetGroup).subscribe(
            (response) => {
                setTimeout(() => {
            ...
            (error) => {
                this.logger.log('error', 'error in getting asset group display name - ' + error);
            },
        );
    }
