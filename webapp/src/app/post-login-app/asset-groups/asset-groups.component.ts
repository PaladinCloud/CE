...
export class AssetGroupsComponent implements AfterViewInit, OnDestroy {
    ...
    openAssetGroupSelector() {
        ...
        this.dialogRef.afterClosed().subscribe((result) => {
            if (result == 'yes') this.setDefault(this.currentAssetTile.toLowerCase());
            else this.instructParentToCloseAssetGroup();
        });
    }
    ...
    updateDefaultAssetGroupForUser(assetGroup) {
        try {
            ...
            this.assetTilesSubscription = this.assetGroupsService
                .updateDefaultAssetGroupForUser(
                    updateAssetGroupUrl,
                    updateAssetGroupMethod,
                    assetGroup,
                    userId,
                )
                .subscribe(
    ...
    getSelectedTabAssets(assettileType) {
        if (this.selectedTabName && assettileType) {
            if (assettileType == 'stakeholder') assettileType = 'user';
    ...
    getAssetDetailTiles(groupName) {
        ...
        if (queryParams['ag'] !== undefined) {
            this.assetDetailsSubscription = this.assetGroupsService
    ...
    updateRecentAssetGroup(groupName) {
        ...
        if (queryParams['ag'] !== undefined) {
            this.updateRecentAGSubscription = this.updateRecentAGService
                .updateRecentlyViewedAG(queryParams, updateRecentAGUrl, updateRecentAGMethod)
                .subscribe(
                    (response) => {
    ...
    checkIsRecentlyViewedOrAll() {
        if (
            ...
            (this.selectedTabName.toLowerCase() === 'recently viewed' ||
                this.selectedTabName.toLowerCase() === 'all')
        ) {
            return true;
        }
        return false;
