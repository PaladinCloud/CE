...
export class OmniSearchDetailsComponent implements OnInit, OnDestroy {
    ...
    numOfCardShown = 1000; // -> To show number of cards appeared in the view(infinite scroll)
            ...
            this.subscriptionDomain = this.domainObservableService
                ...
                .subscribe((domain) => {
                    ...
                    if (this.dataStore.get('OmniSearchCategories')) {
                        this.dropdownData = JSON.parse(this.dataStore.get('OmniSearchCategories'));
    ...
    getData() {
        ...
        try {
            ...
            if (
                ...
                this.terminatedIsChecked = JSON.parse(this.dataStore.get('terminated-cliked'));
            } else {
            ...
            if (
                ...
                this.filterQuery = JSON.parse(this.dataStore.get('omnisearchLastAppliedFilter'));
            }
    ...
    //   this.cbArr = [{'Violation ID': {text: rowDetails._id }, 'Status': {text: rowDetails.issueStatus}}];
    // }
    fetchOmniSearchData() {
        /**
        ...
        if (Object.keys(this.filterQuery).length === 0 && this.filterQuery.constructor === Object) {
            this.fetchFilterdata();
        }
    }
                ...
                (response) => {
                    try {
                        ...
                        if (this.utils.checkIfAPIReturnedDataIsEmpty(response.data)) {
                            this.getErrorValues();
                            this.errorMessage = 'additionalInfo';
                        } else {
    ...
    fetchFilterdata() {
        ...
        this.filterSubscription = this.omniSearchDataService
            .getOmniSearchData(omniSearchUrl, omniSearchMethod, omniSearchResultsPayload)
            .subscribe(
                (response) => {
                    try {
                        this.stopPreviousDataSubscription = false;
    ...
    processHttpResponse(response) {
        try {
            if (this.utils.checkIfAPIReturnedDataIsEmpty(response.data.results)) {
                this.getErrorValues();
    ...
    setOmniSearchCategory() {
        try {
            const omniSearchomniSearchCategoriesUrl = environment.omniSearchCategories.url;
    ...
    processFilterOptions(data) {
        try {
            ...
            if (!this.utils.isObjectEmpty(data)) {
                ...
                let filterValues = data.groupBy.values;
                for (let i = 0; i < filterValues.length; i++) {
                    if (filterValues[i].count == 0) {
                        filterValues[i].applied = false;
                    } else {
                        filterValues[i].applied = true;
                        break;
                    }
                    if (i == filterValues.length - 1) {
                        filterValues[0].applied = true;
                    }
                }
                data.groupBy.values = filterValues;
                this.filterData = data;
                this.filterQuery = data;
                this.dataStore.set('omnisearchLastAppliedFilter', JSON.stringify(this.filterQuery));
                /**
    ...
    filterOptionClicked(event) {
        try {
            ...
            this.dataStore.set('omnisearchLastAppliedFilter', JSON.stringify(this.filterQuery));
            // size of payload results , everytime filter is clicked we are resetting the variable
    ...
    searchCalled(event) {
        try {
            ...
            this.router.navigate(['../../', queryData.filterValue, queryData.searchText], {
                relativeTo: this.activatedRoute,
                queryParamsHandling: 'merge',
            });
            /**
             * SearchText/searchboxValueSelected should get updated by fetchQueryParams func
             * right now fetchQueryParams() is getting called after this.updatecomponent
