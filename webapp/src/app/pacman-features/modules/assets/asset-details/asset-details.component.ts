...
@Component({
    ...
    providers: [
        ...
        HostVulnerabilitiesSummaryService,
        CpuUtilizationService,
        DiskUtilizationService,
        AssetCostService,
    ],
})
export class AssetDetailsComponent implements OnInit, AfterViewInit, OnDestroy {
            ...
            .subscribe(
                (response) => {
                    ...
                    for (let i = 0; i < this.users.length; i++) {
                        const userdetails =
                            this.users[i].displayName + ' ' + '(' + this.users[i].userEmail + ')';
    ...
    updateComponent() {
        try {
            ...
            if (this.activatedRoute.snapshot.queryParams) {
                this.filterText =
                    this.activatedRoute.snapshot.queryParams; /* <-- filterText is used to hit the api filter object */
            }
        } catch (error) {
    ...
    getAwsNotificationData() {
        ...
        const newUrl = this.replaceUrl(url);
        this.awsNotificationSubscription = this.commonResponseService
            .getData(newUrl, method, {}, {})
            .subscribe(
                (response) => {
                    try {
    ...
    getAssetDetailsData() {
        ...
        this.assetDetailsSubscription = this.commonResponseService
            .getData(newUrl, method, {}, {})
            .subscribe(
                (response) => {
                    try {
                        ...
                        if (response.attributes.length > 0) {
                            ...
                            const formattedAttributes = attributes.map(function (attribute) {
                                attribute.name =
                                    refactoredService.getDisplayNameForAKey(attribute.name) ||
                                    attribute.name;
                                return attribute;
                            });
    ...
    relatedData(data) {
        try {
            ...
            Object.keys(this.genericAttributes).map((key) => {
                ...
                const assetIdDisplayNameObject = values.find(
                    (item) => item.name === 'assetIdDisplayName',
                );
    ...
    getPolicyData() {
        ...
        const newUrl = this.replaceUrl(url);
        this.getPolicyDataSubscription = this.policyViolationSummaryService
            .getData(newUrl, method)
            .subscribe(
                (response) => {
                    try {
    ...
    getHostData() {
        ...
        const newUrl = this.replaceUrl(url);
        this.getHostDataSubscription = this.hostVulnerabilitiesSummaryService
            .getData(newUrl, method)
            .subscribe(
                (response) => {
                    try {
                        if (
                            !this.utilityService.checkIfAPIReturnedDataIsEmpty(response) &&
    ...
    getdiskData() {
        ...
        const newUrl = this.replaceUrl(url);
        this.getDiskDataSubscription = this.diskUtilizationService
            .getData(newUrl, method)
            .subscribe(
                (response) => {
                    try {
    ...
    getAccessGroupData() {
        ...
        const payload = {};
        const queryParams = {};
        this.accessGroupSubscription = this.commonResponseService
            .getData(newUrl, method, payload, queryParams)
            .subscribe(
                (response) => {
                    try {
                        if (response.length > 0) {
    ...
    navigateDataTable(event) {
        ...
        try {
            ...
            const eachParams = {
                'severity.keyword': queryObj.toLowerCase(),
                '_resourceid.keyword': this.resourceId,
                'targetType.keyword': this.resourceType,
            };
    ...
    navigateToVulnerabilitiesList(event) {
        try {
            ...
            const newParams = this.utilityService.makeFilterObj(eachParams);
            if (queryObj !== undefined) {
                this.router.navigate(['../../../../', 'compliance', 'vulnerabilities'], {
                    relativeTo: this.activatedRoute,
                    queryParams: newParams,
                    queryParamsHandling: 'merge',
                });
    ...
    postEmail(emailArrayList) {
        ...
        const payload = {
            ...
            placeholderValues: {
                link: locationValue,
                resourceId: this.decodedResourceId,
    ...
    navigateToAWSNotifications(status) {
        ...
        this.router.navigate(['../../../../compliance/health-notifications'], {
            relativeTo: this.activatedRoute,
            queryParams: this.utilityService.makeFilterObj(eachParams),
            queryParamsHandling: 'merge',
        });
    }
