...
export class AssetListComponent implements OnInit, OnDestroy {
    ...
    handleFilterSearchTextChange(event) {
        if (event.selectedFilterCategory == 'Asset ID')
                ...
                .subscribe(
                    async (response) => {
                        response[0].response.forEach((item) => {
                            ...
                            item.optionValue = item.optionValue.includes('resourceType')
                                ? '_entitytype.keyword'
                                : item.optionValue;
                        });
                        ...
                        let filterTypeLabels = this.filterTypeOptions.map(
                            (labelObj) => labelObj.optionName,
                        );
                        ...
                        [this.columnNamesMap, this.columnWidths] =
                            this.utils.getColumnNamesMapAndColumnWidthsMap(
                                ...
                                this.columnWidths,
                                this.columnNamesMap,
                                ['Exempted', 'Tagged', 'Policy ID', 'Compliant'],
                            );
                        this.routerParam();
