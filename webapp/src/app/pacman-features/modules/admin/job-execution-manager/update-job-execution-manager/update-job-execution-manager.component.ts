...
export class UpdateJobExecutionManagerComponent implements OnInit, OnDestroy {
    ...
    private buildUpdateJobModel(jobForm) {
        ...
        this.uploadService
            .pushFileToStorage(url, method, this.currentFileUpload, existingJobModel)
            .subscribe(
                (event) => {
    ...
    getJobDetails() {
        ...
        this.adminService.executeHttpAction(url, method, {}, { jobId: this.jobId }).subscribe(
            (jobDetailsResponse) => {
                ...
                const frequencyforEdit = this.adminUtilityService.decodeCronExpression(
                    this.jobDetails['jobFrequency'],
                );
                this.jobFrequency = [
                    { text: frequencyforEdit.interval, id: frequencyforEdit.interval },
                ];
                ...
                if (frequencyforEdit.interval.toLowerCase() === 'yearly') {
                    ...
                    this.jobFrequencyMonth =
                        this.allMonths[parseInt(frequencyforEdit.month, 10) - 1].text;
