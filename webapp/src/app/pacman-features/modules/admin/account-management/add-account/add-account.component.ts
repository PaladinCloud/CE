...
export class AddAccountComponent implements OnInit, AfterViewInit {
    ...
    videoSource = '';
    redHatStepsVideo =
        'https://paladincloud-production-video.s3.amazonaws.com/redhat-setup-video.mp4';
    tenableStepsVideo =
        'https://paladincloud-production-video.s3.amazonaws.com/tenable-setup-video.mp4';
        ...
        this.activatedRoute.queryParams.subscribe((params) => {
            ...
            if (this.pluginSelected) {
                ...
                const selectedAccount = this.accountsList.find(
                    (account) => account.name === selectedAcc,
                );
    ...
    displayTemplate(addDetailsStepperIndex?) {
        ...
        switch (this.currentStepperIndex) {
            case 0:
                if (
                    currentAccount == 'azure' ||
                    currentAccount == 'aqua' ||
                    currentAccount == 'qualys' ||
                    currentAccount == 'gcp' ||
                    currentAccount == 'red hat'
                )
                ...
                break;
            case 1:
                if (currentAccount == 'aws') this.currentTemplateRef = this.configureAccountRef;
    ...
    public buildForm() {
        this.awsPluginForm = this.form.group({
            accountId: ['', [Validators.required, CustomValidators.exactDigits]],
            accountName: [''],
        });
        ...
        this.azurePluginForm = this.form.group({
            applicationId: [
                '',
                [Validators.required, CustomValidators.alphanumericHyphenValidator],
            ],
        ...
        this.gcpPluginForm = this.form.group({
            projectId: [
                ...
                [
                    Validators.required,
                    CustomValidators.alphanumericHyphenValidator,
                    CustomValidators.validateProjectId,
                ],
            ],
    ...
    shouldHideAddButton(): boolean {
        ...
        return (
            (this.pluginSelected === 'gcp' &&
                (this.currentStepperIndex < 2 || this.currentStepperIndex > 2)) ||
            (this.pluginSelected !== 'gcp' &&
                (this.currentStepperIndex < 1 || this.currentStepperIndex > 1))
        );
    }
    ...
    isNextButtonDisabled(): boolean {
        return (
            this.selectedAccount &&
            this.selectedAccount.toLowerCase() === 'aws' &&
            this.currentPluginForm?.invalid
        );
    }
    ...
    getRoleName() {
        ...
        this.baseAccountSubscription = this.commonResponseService
            .getData(url, method, {}, {})
            .subscribe((response) => {
                try {
    ...
    validateAccount() {
        ...
        this.validateSubscription = this.commonResponseService
    ...
    createCommand() {
        this.firstGcpCommand =
            'cloud iam workload-identity-pools create ' +
            this.workloadIdentityPoolId +
            ' --location=' +
            this.location +
            " --description='AWS id pool 2' --display-name='AWS workload id federation'";
        this.secondGcpCommand =
