...
export class AccountManagementComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    columnWidths: IColumnWidthsMap = {
        [TABLE_COLUMN_NAMES.ACCOUNT_NAME]: 1.5,
        [TABLE_COLUMN_NAMES.ACCOUNT_ID]: 1.5,
        [TABLE_COLUMN_NAMES.ASSETS]: 0.5,
        [TABLE_COLUMN_NAMES.VIOLATIONS]: 0.5,
        [TABLE_COLUMN_NAMES.STATUS]: 0.5,
        [TABLE_COLUMN_NAMES.CREATED_BY]: 1,
        [TABLE_COLUMN_NAMES.CREATED_DATE]: 1,
    };
    whiteListColumns;
    tableScrollTop = 0;
    centeredColumns = {
    ...
    getBaseAccount() {
        ...
        this.commonResponseService.getData(url, method, {}, {}).subscribe((response) => {
            try {
    ...
    processData(data) {
        try {
            return this.utils.processTableData(
                data,
                ...
                (row, col, cellObj) => {
                    if (col === TABLE_COLUMN_NAMES.ACCOUNT_NAME) {
                        ...
                        cellObj = {
                            ...cellObj,
                            chipList:
                                row[col].toLowerCase() === 'configured' ? ['Online'] : ['Offline'],
            ...
            .subscribe((responseData) => {
                try {
                    ...
                    if (status.toLowerCase() == 'success') {
                        nofificationMessage =
                            'Account ' + accountId + ' has been deleted successfully';
    ...
    getFilterPayloadForDataAPI() {
        ...
        Object.keys(filterToBePassed).forEach((filterKey) => {
            ...
            if (this.dateCategoryList.includes(this.columnNamesMap[filterKey])) {
                ...
                const dateRange = [
                    this.utils.getFormattedDate(fromDate, false, 'z'),
                    this.utils.getFormattedDate(toDate, true, 'z'),
                ];
                filterToBePassed[filterKey] = dateRange;
            } else if (
                this.columnNamesMap[filterKey] === TABLE_COLUMN_NAMES.ASSETS ||
                this.columnNamesMap[filterKey] === TABLE_COLUMN_NAMES.VIOLATIONS
            ) {
