...
export class ConfigUsersComponent implements OnInit, OnDestroy {
                ...
                .subscribe(
                    (response) => {
                        ...
                        for (let i = 0; i < this.users.length; i++) {
                            const userdetails =
                                this.users[i].userName + ' ' + '(' + this.users[i].userId + ')';
                            this.users[i]['user'] = userdetails;
                            if (
                                this.allAllocatedUsers.indexOf(
                                    this.users[i].userId.toLowerCase(),
                                ) === -1
                            ) {
    ...
    createRole(roleDetails) {
        ...
        this.adminService.executeHttpAction(url, method, roleDetails, {}).subscribe(
            (reponse) => {
    ...
    configureUsers() {
        ...
        this.adminService.executeHttpAction(url, method, userRoleConfigRequest, {}).subscribe(
            (reponse) => {
    ...
    getRoleDetails() {
        ...
        this.adminService.executeHttpAction(url, method, {}, { roleId: this.roleId }).subscribe(
            (userRoleReponse) => {
                if (!this.isCreate) {
