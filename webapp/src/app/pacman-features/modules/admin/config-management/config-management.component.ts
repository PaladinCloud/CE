...
export class ConfigManagementComponent implements OnInit, OnDestroy {
    ...
    storeFirstLevel(data, index, event?) {
        try {
            ...
            if (data == null || data.length === 0) {
                ...
                this.firstLevelData['properties'] = uniqBy(
                    this.firstLevelData['properties'],
                    'key',
                );
    ...
    storeSecondLevel(data, index) {
        try {
            ...
            if (data == null || data.length === 0) {
                ...
                // remove duplicates from properties
                this.secondaryLevelData['properties'] = uniqBy(
                    this.secondaryLevelData['properties'],
                    'key',
                );
    ...
    storeThirdLevel(data, index) {
        try {
            ...
            // remove duplicates from properties
            this.tertiaryLevelData['properties'] = uniqBy(
                this.tertiaryLevelData['properties'],
                'key',
            );
            this.applicationBreadcrumb = [];
            this.applicationBreadcrumb.push(
    ...
    register(myForm: NgForm) {
        ...
        if (this.configManagementForm.controls.hasOwnProperty('overidden')) {
            const overridenFormGroup = this.configManagementForm['controls'][
                'overidden'
            ] as FormGroup;
    ...
    transactionalMessage() {
        if (this.updateInProgress) {
            if (this.transactionResponse.updatedConfig.success) {
                ...
                this.transactionObj.message =
                    'Succesfully updated configuration data for ' + this.editData.name;
            } else if (this.transactionResponse.updatedConfig.error) {
                this.transactionObj.type = 'error';
                this.transactionObj.title = 'Error!';
                this.transactionObj.message =
                    'Updating configuration data for ' + this.editData.name;
            }
            ...
            if (this.transactionResponse.createdConfig.success) {
                ...
                this.transactionObj.obj1.message =
                    'Succesfully created configuration data for ' + this.editData.name;
                ...
                this.transactionObj.obj1.message =
                    'Creating configuration data for ' + this.editData.name;
            }
    ...
    allMessages() {
        if (
            ...
            this.editData.properties.length > 0 &&
            this.overiddenData.length > 0
        ) {
            ...
            if (this.showConfBox.resetToView) {
                ...
                if (this.showConfBox.isEditPageCancelBtnClick) {
                    ...
                    if (this.showConfBox.modified.columnIndex === 1) {
                        ...
                        this.storeSecondLevel(
                            this.firstLevelData['children'][this.applyActive2],
                            this.applyActive2,
                        );
                    } else if (this.showConfBox.modified.columnIndex === 3) {
                        this.storeThirdLevel(
                            this.secondaryLevelData['children'][this.applyActive3],
                            this.applyActive3,
                        );
                    }
                }
    ...
    onOverride(data) {
        ...
        let individualField = {
            ...
            formControlDisplayName:
                this.refactorFieldsService.getDisplayNameForAKey(data.key.toLocaleLowerCase()) ||
                data.key,
        };
        if (data.description) {
        ...
        this.toastObservableService.postMessage(
            'Field added to ' + this.editData.name + ' to override ',
            3,
        );
    }
        ...
        try {
            this.deletePayLoadOnhold = {
                ...
                payload: {
                    ...
                    configValue:
                        this.configManagementForm.controls[fieldType]['controls'][
    ...
    onDeleteNotSavedField(fieldType, input) {
        try {
            ...
            this.overrideFormData = this.overrideFormData.filter(
                (item) => item.formControlName !== input.formControlName,
            );
        } catch (error) {
    ...
    deleteFieldOnConfirm(deleteObj) {
        ...
        this.deleteTransactionResponse.message = 'Succesfully deleted configuration: ' + this.deletePayLoadOnhold['payload']['configKey'];*/
                ...
                (error) => {
                    ...
                    this.deleteTransactionResponse.message =
                        'Deleting configuration ' +
                        this.deletePayLoadOnhold['payload']['configKey'];
