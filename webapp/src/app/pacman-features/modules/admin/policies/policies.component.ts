...
export class PoliciesComponent implements OnInit, OnDestroy {
    ...
    filterFunctionMap = {
        'Autofix status': (item, filterKey, filterValue) => {
            if (filterValue == 'available') {
                return (
                    item[filterKey].valueText.toLowerCase() == 'enabled' ||
                    item[filterKey].valueText.toLowerCase() == 'available'
                );
            ...
            if (value.toLowerCase() === 'asset type') {
                this.assetTypeMapService.getAssetMap().subscribe((assetTypeMap) => {
                    filterTagsData.forEach((filterOption) => {
                        filterOption['name'] =
                            assetTypeMap.get(filterOption['name']?.toLowerCase()) ||
                            filterOption['name'];
                    });
                });
            }
    ...
    getPolicyDetails(isNextPageCalled?) {
        ...
        try {
            this.dataSubscription = this.adminService
                .executeHttpAction(url, method, payload, {})
                .subscribe(
                    (reponse) => {
                ...
                (row, col, cellObj) => {
                    if (col.toLowerCase() == 'policy') {
                        ...
                        if (autoFixAvailable == 'true') {
                            ...
                            imageTitleText =
                                autoFixEnabled == 'true' ? 'Autofix Enabled' : 'Autofix Available';
                        }
                        cellObj = {
                            ...cellObj,
                            imgSrc: imgSrc,
    ...
    openDialog(event): void {
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result == 'yes') {
                ...
                } else this.enabelOrDisableAutofix(policyId, autofix);
            }
        });
    }
    ...
    enabelOrDisableAutofix(policyId: string, autoFix: string) {
        try {
            ...
            this.adminService
                .executeHttpAction(url, method, {}, queryParams)
                .subscribe((response) => {
                    if (response && response[0].message == 'success') {
                        const snackbarText =
                            'Autofix for policy "' + policyId + '" ' + autoFix + 'd successfully';
                        this.openSnackBar(snackbarText, 'check-circle');
                        this.getPolicyDetails();
                    }
                });
    ...
    invokePolicy(policyId: string) {
        ...
        this.adminService.executeHttpAction(url, method, [{}], { policyId: policyId }).subscribe(
            (response) => {
                ...
                if (invocationId)
                    this.openSnackBar(
                        'Invocation Id ' + invocationId + ' invoked successfully!!',
                        'check-circle',
                    );
            },
            (error) => {
