...
export class UserManagementComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    getPreservedState() {
        ...
        if (state) {
            ...
            this.whiteListColumns = state?.whiteListColumns ?? [
                'Email',
                'Roles',
                'Status',
                'Actions',
    ...
    createEditUser(currentRow: any) {
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result == 'yes') {
                this.resetComponent();
                if (currentRow) this.updateUserRoles(this.selectedRoles);
                else {
                    this.addNewUser();
                }
            }
    ...
    updateUserRoles(existingRoles: string[]) {
        ...
        this.adminService.executeHttpAction(newUrl, method, payload, {}).subscribe((response) => {
            try {
                if (response) {
                    this.getUserList();
                    if (this.action == 'Edit') {
    ...
    deleteUser(username: string, existingRoles: any) {
        ...
        this.adminService.executeHttpAction(newUrl, method, {}, {}).subscribe((response) => {
            try {
                if (response) {
                    this.getUserList();
        ...
        dialogRef.afterClosed().subscribe((result) => {
            try {
                if (result == 'yes') {
                    ...
                    } else this.userActivation(username, roles);
                }
            } catch (error) {
                this.errorHandling.handleJavascriptError(error);
            }
    ...
    userActivation(username: string, existingRoles: any) {
        ...
        this.adminService.executeHttpAction(replacedUrl, method, {}, {}).subscribe((response) => {
            try {
                if (response[0].message == 'success') {
                    this.getUserList();
                    this.openSnackBar(
                        'User status ' + this.action.toLowerCase() + 'd successfully',
                        'check-circle',
                    );
                }
            } catch (error) {
    ...
    getUserList(isNextPageCalled?) {
        ...
        if (this.filters.length && this.filters[0].key && this.filters[0].value) {
            queryParams = {
                ...
                filter:
                    'status = "' +
                    (this.filters[0].value == 'active' ? 'Enabled' : 'Disabled') +
                    '"',
            };
        }
