...
export class CreateStickyExceptionsComponent implements OnInit, OnDestroy {
    ...
    addAttributes(attributeName, attributeValue) {
        let ruleDetails = find(this.allAttributeDetails[this.selectedIndex].allPolicies, {
            text: attributeName,
        });
        this.allAttributeDetails[this.selectedIndex].policies.push(ruleDetails);
        let itemIndex =
            this.allAttributeDetails[this.selectedIndex].allPolicies.indexOf(ruleDetails);
        if (itemIndex !== -1) {
    ...
    isExceptionNameAvailable(exceptionNameKeyword) {
        if (exceptionNameKeyword.trim().length < 5) {
            ...
            let isKeywordExits = this.exceptionNames.findIndex(
                (item) => exceptionNameKeyword.trim().toLowerCase() === item.trim().toLowerCase(),
            );
            if (isKeywordExits === -1) {
                this.isExceptionNameValid = 1;
            } else {
    ...
    getAllStickyExceptionDetails(exceptionName) {
        ...
        this.adminService
            .executeHttpAction(url, method, {}, { exceptionName: exceptionName, dataSource: 'aws' })
            .subscribe(
                (reponse) => {
                    this.assetLoader = false;
                    this.pageContent[0].hide = false;
                    ...
                    this.exceptionDetailsForm = {
                        ...
                        expiry: this.datePipe.transform(
                            reponse[0].expiryDate,
                            this.exceptionDetailsExpiryDateFormat,
                        ),
    ...
    private collectTargetTypes() {
        ...
        this.adminService
            .executeHttpAction(url, method, {}, { assetGroupName: assetGroupName })
            .subscribe(
                (reponse) => {
                    this.assetLoader = false;
                    this.showLoader = false;
    ...
    createException(exceptionFormDetails) {
        ...
        this.adminService
            .executeHttpAction(url, method, { ...exceptionDetails, createdBy: userId }, {})
            .subscribe(
                (reponse) => {
    ...
    deleteException(exceptionDetails) {
        ...
        this.adminService.executeHttpAction(url, method, exceptionDetails, {}).subscribe(
            (reponse) => {
    ...
    routerParam() {
        try {
            ...
            let currentQueryParams = this.routerUtilityService.getQueryParametersFromSnapshot(
                this.router.routerState.snapshot.root,
            );
...
function getReqDetailsForLogging(exceptionFormDetails: any) {
    let targetTypesAndPoliciesIncluded = exceptionFormDetails.targetTypes
        ?.filter((obj) => obj.policies.length > 0)
        .map((obj) => {
            let returnObj = { ...obj };
