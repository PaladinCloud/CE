...
export class CreateEditPolicyComponent implements OnInit, OnDestroy {
    ...
    isSelectedAssetGroupRedHat: boolean = false;
    selectedMonths: any;
    selectedDays: any;
    ...
    buildAutofixForm() {
        const maxEmailNotificationValue = parseInt(
            (this.waitingTime / this.jobInterval).toString(),
            10,
        );
        this.countTooltipText =
            'Count should not exceed the limit value ' + maxEmailNotificationValue;
        this.notificationsForm = this.form.group({
            waitingTime: [
                this.waitingTime,
                [Validators.required, CustomValidators.checkIfMultipleOfTwentyFour],
            ],
            maxEmailNotification: [
                this.maxEmailNotification,
                [Validators.required, Validators.max(maxEmailNotificationValue)],
            ],
        });
        this.notificationsForm.get('maxEmailNotification').markAsTouched();
        this.notificationsForm.get('waitingTime').markAsTouched();

        this.waitingTimeSubscription = this.notificationsForm
            .get('waitingTime')
            .valueChanges.subscribe((newValue) => {
                const maxEmailNotificationValue = parseInt(
                    (newValue / this.jobInterval).toString(),
                    10,
                );
                this.countTooltipText =
                    'Count should not exceed the limit value ' + maxEmailNotificationValue;
                this.notificationsForm
                    .get('maxEmailNotification')
                    .setValidators([
                        Validators.required,
                        Validators.max(maxEmailNotificationValue),
                    ]);
    ...
    createOrUpdatepolicy(PolicyModel: any) {
        ...
        this.uploadService
            .pushFileToStorage(url, method, this.currentFileUpload, PolicyModel)
            .subscribe(
                (event) => {
                    this.getData();
                    ...
                    const notificationMessage =
                        'Policy ' + this.policyDisplayName + ' updated successfully!!';
    ...
    getTargetTypeNamesByDatasourceName(datasourceName) {
        ...
        this.adminService
            .executeHttpAction(url, method, {}, { dataSourceName: datasourceName })
            .subscribe(
                (reponse) => {
                    this.showLoader = false;
    ...
    getPolicyDetails(policyId: any, action?) {
        ...
        this.adminService.executeHttpAction(url, method, {}, { policyId: policyId }).subscribe(
            (reponse) => {
                ...
                this.isSelectedAssetGroupRedHat = this.selectedAssetGroup == 'redhat';
                ...
                if (this.isAutofixAvailable) {
                    this.getAccounts();
                    this.selectedAccounts = this.policyDetails.allowList
                        ? this.policyDetails.allowList.split(',').slice()
                        : [];
                    ...
                    this.waitingTime = this.policyDetails.waitingTime
                        ? this.policyDetails.waitingTime
                        : 0;
                    this.maxEmailNotification = this.policyDetails.maxEmailNotification
                        ? this.policyDetails.maxEmailNotification
                        : 0;
                ...
                this.allPolicyParams =
                    JSON.parse(this.policyDetails.policyParams || '{}')['params'] || [];
                this.paramsList = [];
                for (let i = this.allPolicyParams.length - 1; i >= 0; i -= 1) {
                    ...
                    if (
                        this.allPolicyParams[i]['key'].toLowerCase() == 'policycategory' ||
                        this.allPolicyParams[i]['key'].toLowerCase() == 'severity'
                    ) {
                        continue;
                    }
                    this.paramsList.push({
                        ...
                        displayName: this.allPolicyParams[i]['displayName']
                            ? this.allPolicyParams[i]['displayName']
                            : this.allPolicyParams[i]['key'],
                        isEdit: this.allPolicyParams[i]['isEdit']
                            ? JSON.parse(this.allPolicyParams[i]['isEdit'])
                            : false,
                        isMandatory: this.allPolicyParams[i]['isMandatory']
                            ? this.allPolicyParams[i]['isMandatory']
                            : false,
    ...
    openDialog(PolicyModel: any): void {
        ...
        const dialogRef = this.dialog.open(DialogBoxComponent, {
            width: '500px',
            data: { title: title, yesButtonLabel: yesButtonLabel },
        });
    ...
    toggleStatus(event: any) {
        ...
        if (!this.status) this.isAutofixEnabled = this.status;
    }
    ...
    enableDisableRuleOrJob(action: string) {
        return new Promise((resolve, reject) => {
            try {
                ...
                this.adminService.executeHttpAction(url, method, payload, params).subscribe(
                    (response) => {
