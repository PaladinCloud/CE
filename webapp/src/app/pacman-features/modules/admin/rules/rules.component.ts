...
export class RulesComponent implements OnInit, OnDestroy {
    ...
    massageData(data) {
        ...
        let formattedFilters = data.map(function (data) {
            ...
            keysTobeChanged.forEach((element) => {
                var elementnew = refactoredService.getDisplayNameForAKey(element) || element;
    ...
    isRuleEnableAccess(roleCapabilities, ruleCategory) {
        ...
        if (
            roleCapabilities.includes('rules-security') &&
            roleCapabilities.includes('rules-technical-admin')
        ) {
            return true;
        } else {
            ...
            for (var row = 0; row < getData.length; row++) {
                ...
                for (var col = 0; col < getCols.length; col++) {
                    if (getCols[col].toLowerCase() == 'actions') {
                        ...
                        let allowEnableDisable = this.isRuleEnableAccess(
                            roleCapabilities,
                            ruleCategory,
                        );
