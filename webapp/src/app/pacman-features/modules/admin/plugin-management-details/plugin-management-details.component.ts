...
export class PluginManagementDetailsComponent implements OnInit, OnDestroy {
    ...
    buildForm(plugin) {
        ...
        for (let i = 0; i < pluginDetails.length; i++) {
            ...
            this.formGroup[pluginDetails[i].key] = new FormControl(
                pluginDetails[i].value,
                Validators.required,
            );
    ...
    operateList(updatedValues) {
        {
            ...
            try {
                ...
                const updatedPluginDetailsObj = existingPluginObj.pluginDetails.map(
                    (pluginDetailField, index) => {
