...
export class InvokeRuleComponent implements OnInit, OnDestroy {
    ...
    invokeRule() {
        ...
        this.adminService
            .executeHttpAction(url, method, this.allOptionalRuleParams, { policyId: this.policyId })
            .subscribe(
                (reponse) => {
    ...
    addOptionalRuleParameters(ruleParamKey: any, ruleParamValue: any, isEncrypted: any) {
        if (ruleParamKey.value !== '' && ruleParamValue.value !== '') {
            this.allOptionalRuleParams.push({
                key: ruleParamKey.value,
                value: ruleParamValue.value,
