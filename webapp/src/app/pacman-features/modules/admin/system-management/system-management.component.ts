    ...
    postOperations(jobType, jobAction) {
        ...
        this.systemSubscription = this.commonResponseService
            ...
            .subscribe(
                (response) => {
                    let custom_message =
                        this.titleCasePipe.transform(jobType) +
                        's operation is performed successfully.';
                    if (response) {
                ...
                (error) => {
                    ...
                    this.showLoader = false;
                    error.toLowerCase() !== 'apiresponseerror'
                        ? (this.errorMsg = error)
                        : (this.errorMsg =
                              'Oops! An error occurred while performing the  ' +
                              jobType +
                              ' batches operation.');
                },
            );
