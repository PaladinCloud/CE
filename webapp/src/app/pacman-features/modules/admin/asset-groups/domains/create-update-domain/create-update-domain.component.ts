...
export class CreateUpdateDomainComponent implements OnInit, OnDestroy {
    ...
    createDomain(domain) {
        ...
        this.adminService.executeHttpAction(url, method, domain, {}).subscribe(
            (reponse) => {
    ...
    isDomainNameAvailable(domainNameKeyword) {
        if (domainNameKeyword.trim().length < 5) {
            ...
            let isKeywordExits = this.domainNames.findIndex(
                (item) => domainNameKeyword.trim().toLowerCase() === item.trim().toLowerCase(),
            );
            if (isKeywordExits === -1) {
                this.isDomainNameValid = 1;
            } else {
    ...
    getAllDomainNames() {
        ...
        this.adminService
            .executeHttpAction(url, method, {}, { domainName: this.domainName })
            .subscribe(
                (reponse) => {
                    this.domainLoader = false;
                    this.hideContent = false;
