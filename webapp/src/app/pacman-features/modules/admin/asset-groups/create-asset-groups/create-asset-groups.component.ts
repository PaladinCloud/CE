...
export class CreateAssetGroupsComponent implements OnInit, OnDestroy {
    ...
    public buildForm() {
        this.assetGroupForm = this.form.group({
            assetGroupDisplayName: [
                ...
                [
                    Validators.required,
                    this.isGroupNameAvailable.bind(this),
                    CustomValidators.noStartingNumberOrSpecialCharacter,
                ],
            ],
            assetGroupDesc: [
                '',
                [
                    Validators.required,
                    CustomValidators.minLengthTrimValidator(6),
                    Validators.maxLength(100),
                ],
            ],
            selectedAccountType: [
                '',
                [Validators.required, this.isValidType.bind(this), CustomValidators.onlyAlphabets],
            ],
        });
    }
    ...
    createCriteriaList(criteriaMap) {
        ...
        for (const criteria in criteriaMap) {
            ...
            for (let j = 0; j < conditionList.length; j++) {
                const conditionKey = this.AttributeKeyViewMap[conditionList[j]['attributeName']]
                    ? this.AttributeKeyViewMap[conditionList[j]['attributeName']]
                    : conditionList[j]['attributeName'];
        ...
        for (let i = 0; i < cloudsData.length; i++) {
            cloudsData[i] = Object.entries(cloudsData[i]).reduce((acc, [oldKey, value]) => {
                const newKey = this.AttributeKeyViewMap[oldKey]
                    ? this.AttributeKeyViewMap[oldKey]
                    : oldKey;
                acc[newKey] = value;
                return acc;
            }, {});
    ...
    deleteAssetGroup() {
        ...
        this.adminService.executeHttpAction(url, method, { groupId: this.groupId }, {}).subscribe(
            (response) => {
                if (response) {
                    ...
                    if (response[0].message === 'success') {
                        ...
                        this.criteriaDetails?.forEach((crit) => {
                            if (crit.criteriaName in criteriasBeforeUpdate) {
                                criteriasBeforeUpdate[crit.criteriaName][crit.attributeName] =
                                    crit.attributeValue;
                            } else {
                        ...
                        let agDetails = {
                            ...
                            description: this.assetGroupDesc,
                            type: this.selectedAccountType,
                            configuration: Object.values(criteriasBeforeUpdate),
                        };
            ...
            (error) => {
                this.notificationObservableService.postMessage(
                    'Error in deleting asset group',
                    3000,
                    'error',
                    'Error',
                );
    ...
    getDisplayName(selectedKey: string, selectedValue: string): string {
        ...
        if (selectedKey === 'source') {
            ...
            if (this.assetTypeMap.get(selectedValue)) return this.assetTypeMap.get(selectedValue);
        }
        ...
        if (!attributeDetail.includeAll) {
            ...
            if (this.isCreate) {
                this.selectedAttributeIndex =
                    attributeDetail['index'] +
                    '/_search?filter_path=aggregations.alldata.buckets.key';
            } else if (!this.isCreate && attributeDetail.dataSourceName) {
                dataSourceName = '/' + attributeDetail.dataSourceName + '_';
                this.selectedAttributeIndex =
                    dataSourceName +
                    attributeDetail.targetName +
                    '/_search?filter_path=aggregations.alldata.buckets.key';
            } else
                this.selectedAttributeIndex =
                    dataSourceName +
    ...
    isGroupNameAvailable(control: AbstractControl) {
        ...
        const isKeywordExits = this.assetGroupNames.findIndex(
            (item) => alexaKeyword === item.toLowerCase(),
        );
        if (isKeywordExits === -1) {
            return null;
        }
    ...
    async getAssetGroupsList(): Promise<any[]> {
        try {
            ...
            if (!assetGroupsList) {
                const response: any = await this.assetTilesService
                    .getAssetGroupList()
    ...
    getAllAssetGroupNames() {
        ...
        this.adminService.executeHttpAction(url, method, {}, {}).subscribe(
            (response) => {
    ...
    checkAttributeAlreadyTaken(attributeValue) {
        const attributeSearchedResult = find(
            this.allAttributeDetails[this.selectedIndex].attributes,
            { name: this.attributeName, value: attributeValue },
        );
    ...
    getTargetTypeAttributeValues(attributeName: any) {
        ...
        let queryParams = {
            index: this.selectedAttributeIndex,
            payload: JSON.stringify(attrNameObj),
        };
                ...
                if (attributeValues.length > 0) {
                    if (attributeValues[0].hasOwnProperty('data')) {
                        if (attributeValues[0].data.hasOwnProperty('aggregations')) {
                            if (
                                attributeValues[0].data.aggregations.alldata.hasOwnProperty(
                                    'buckets',
                                )
                            ) {
                                const allAttributeValues =
                                    attributeValues[0].data.aggregations.alldata.buckets;
    ...
    addTagetType(targetTypeValue) {
        ...
        const targetTypeDetails1 = find(this.remainingTargetTypesFullDetails, {
    ...
    addAttributes(attributeName, attributeValue) {
        this.allAttributeDetails[this.selectedIndex].attributes.push({
            name: attributeName,
        ...
        if (!this.isCreate) {
            ...
            if (this.stepIndex + 1 === 1) {
                ...
                if (!this.pageContent[this.stepIndex].isChanged) {
                    ...
                    this.adminService.executeHttpAction(url, method, domainList, {}).subscribe(
                        (reponse) => {
                            this.assetLoader = false;
                            this.showLoader = false;
                            if (reponse !== undefined) {
                                ...
                                if (this.selectedTdItemsCopyForPrevNext.length > 0) {
                                    this.selectedTdItemsCopyForPrevNext.forEach((tdItem) => {
                                        const availableTdSearchedResult = find(
                                            this.availableTdItems,
                                            { targetName: tdItem.targetName },
                                        );
                                        const itemIndex =
                                            this.availableTdItems.indexOf(
                                                availableTdSearchedResult,
                                            );
                                        if (itemIndex !== -1) {
                ...
                if (!this.pageContent[this.stepIndex].isChanged) {
                    ...
                    this.adminService
                        .executeHttpAction(url, method, this.selectedTdItems, {})
                        .subscribe(
                            (reponse) => {
                                this.assetLoader = false;
                                this.showLoader = false;
                                if (reponse !== undefined) {
                                    ...
                                    if (this.allAttributeDetailsCopyForPrevNext.length > 0) {
                                        this.allAttributeDetailsCopyForPrevNext.forEach(
                                            (attrElement) => {
                                                const attributeSearchedResult = find(
                                                    this.allAttributeDetails,
                                                    { targetName: attrElement.targetName },
                                                );
                                                const itemIndex =
                                                    this.allAttributeDetails.indexOf(
                                                        attributeSearchedResult,
                                                    );
                                                if (itemIndex !== -1) {
                                                    this.allAttributeDetails[itemIndex] =
                                                        attrElement;
                                                }
                                            },
                                        );
                                    }
                                    this.allSelectedAttributeDetailsCopy = cloneDeep(
                                        this.allAttributeDetails,
                                    );
                                    this.goToNextStep();
                                }
    ...
    submit() {
        ...
        const url =
            this.submitBtn == 'Confirm and Create'
                ? environment.createAssetGroups.url
                : environment.updateAssetGroups.url;
        ...
        this.criterias.forEach((criteria) => {
            ...
            criteria.forEach((condition) => {
                const conditionKey = this.AttributeKeyMap[condition.selectedKey]
                    ? this.AttributeKeyMap[condition.selectedKey]
        ...
        this.adminService.executeHttpAction(url, method, payload, {}).subscribe(
            (response) => {
                    ...
                    if (response[0]['message'] === 'success') {
                        if (this.submitBtn == 'Confirm and Create') {
                            ...
                            for (let obj in this.configurationsBeforeEdit['configuration']) {
                                this.configurationsBeforeEdit['configuration'][obj]?.forEach(
                                    (crit) => {
                                        if (crit.criteriaName in criteriasBeforeUpdate) {
                                            criteriasBeforeUpdate[crit.criteriaName][
                                                crit.attributeName
                            ...
                            this.configurationsBeforeEdit['configuration'] =
                                Object.values(criteriasBeforeUpdate);
                        }
                        this.notificationObservableService.postMessage(
                            data,
                            3000,
    ...
    isStepDisabled(stepIndex) {
        if (stepIndex === 0) {
            if (
                this.assetForm.groupName.trim().length >= 3 &&
                this.assetForm.displayName.trim().length >= 3 &&
                this.assetForm.type.trim().length >= 5 &&
                this.assetForm.description.trim().length >= 15 &&
                this.assetForm.createdBy.trim().length >= 5 &&
                this.isGroupNameValid === 1 &&
                !ONLY_DIGITS.test(this.assetForm.createdBy.trim())
            ) {
                return false;
            }
        } else if (stepIndex === 1) {
    ...
    moveItemToRight() {
        ...
        for (const choosedSelectedKey in this.availChoosedSelectedItems) {
            if (this.availChoosedSelectedItems.hasOwnProperty(choosedSelectedKey)) {
                ...
                const filterIndex = availableItemsCopy.indexOf(
                    this.availChoosedSelectedItems[choosedSelectedKey],
                );
    ...
    moveItemToLeft() {
        ...
        for (const choosedSelectedKey in this.selectChoosedSelectedItems) {
            if (this.selectChoosedSelectedItems.hasOwnProperty(choosedSelectedKey)) {
                ...
                const filterIndex = selectedItemsCopy.indexOf(
                    this.selectChoosedSelectedItems[choosedSelectedKey],
                );
   ...
   */
    ...
    moveTdItemToRight() {
        ...
        for (const choosedTdSelectedKey in this.availTdChoosedSelectedItems) {
            if (this.availTdChoosedSelectedItems.hasOwnProperty(choosedTdSelectedKey)) {
                ...
                const filterIndex = availableTdItemsCopy.indexOf(
                    this.availTdChoosedSelectedItems[choosedTdSelectedKey],
                );
    ...
    moveTdItemToLeft() {
        ...
        for (const choosedTdSelectedKey in this.selectTdChoosedSelectedItems) {
            if (this.selectTdChoosedSelectedItems.hasOwnProperty(choosedTdSelectedKey)) {
                ...
                const filterIndex = selectedTdItemsCopy.indexOf(
                    this.selectTdChoosedSelectedItems[choosedTdSelectedKey],
                );
    ...
    getAssetGroupDetails() {
        ...
        this.adminService
            .executeHttpAction(url, method, {}, { assetGroupId: this.groupId, dataSource: '' })
            .subscribe(
                (assetGroupReponse) => {
