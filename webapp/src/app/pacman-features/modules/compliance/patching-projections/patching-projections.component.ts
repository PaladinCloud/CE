...
export class PatchingProjectionsComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.subscriptionToAssetGroup = this.assetGroupObservableService
            ...
            .subscribe((assetGroupName) => {
                ...
                this.numberOfWeeks = utilsService.getNumberOfWeeks(
                    this.yearSelected,
                    this.quarterSelected,
                );
                this.weekProjections = this.createObjectToIterateUpon(this.numberOfWeeks);
                this.storeProjectionData = JSON.parse(
                    JSON.stringify(this.createObjectToIterateUpon(this.numberOfWeeks)),
                );
                this.getTargetTypes();
            });
    }
    ...
    getProjections() {
        ...
        this.numberOfWeeks = this.utilsService.getNumberOfWeeks(
            this.yearSelected,
            this.quarterSelected,
        );
        this.weekProjections = this.createObjectToIterateUpon(this.numberOfWeeks);
        ...
        this.targetSubscription = this.commonResponseService
                ...
                (response) => {
                    try {
                        ...
                        if (response.projectionByWeek.length === 0) {
                            ...
                            this.weekProjections = this.checkifProjectionEqualsWeeks(
                                this.weekProjections,
                            );
                            this.totalweeksProjection = 0;
                            this.weekProjections.forEach((element) => {
                                this.totalweeksProjection =
                                    this.totalweeksProjection + element.projection;
                            });
                            this.storeProjectionData = JSON.parse(
                                JSON.stringify(response.projectionByWeek),
                            );
                        }
                    } catch (e) {
                        this.errorValue = -1;
    ...
    clickClearDropdown() {
        setTimeout(function () {
            ...
            const clear = element.getElementsByClassName('btn btn-xs btn-link pull-right');
