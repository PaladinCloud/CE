...
export class RecommendationsComponent implements OnInit, OnChanges, OnDestroy {
            ...
            .then((success) => {
                ...
                if (
                    ...
                    this.selectedApplication !==
                        this.appliedFilters.pageLevelAppliedFilters['tags.Application.keyword']
                ) {
                    this.updateComponent();
                }
    ...
    getSummaryData() {
        try {
            ...
            this.summarySubscription = this.commonResponseService
    ...
    processSummary(response) {
        ...
        if (this.appliedFilters.queryParamsWithoutFilter['category'] || this.selectedTabName) {
            ...
            category = this.checkSelectedCategoryAvailable(currentCategory)
                ? this.appliedFilters.queryParamsWithoutFilter['category']
                : this.selectTab(this.tabName[0]);
        } else if (!this.selectedTabName) {
            this.activatedRoute.queryParams.subscribe((params) => {
                categoryType = params['category'];
            });

            const selectedTabFromList = this.tabName.find(
                (element) => element.category === categoryType,
            );
    ...
    processTableData(response) {
        ...
        header.forEach((element) => {
            this.tableListData['header'].push(
                this.refactorFieldsService.getDisplayNameForAKey(element.toLocaleLowerCase()) ||
                    element,
            );
        });
    ...
    getApplicationCost() {
        try {
            ...
            this.costSubscription = this.commonResponseService
                .getData(url, method, payload, queryParam)
                .subscribe(
                    (response) => {
    ...
    showHelpContent(event) {
        ...
        this.router.navigate(['/pl', { outlets: { helpTextModal: ['help-text'] } }], {
