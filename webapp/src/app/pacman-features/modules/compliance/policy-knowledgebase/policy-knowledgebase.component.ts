...
export class PolicyKnowledgebaseComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    getPreservedState() {
        ...
        if (state?.policyCategoryDic) this.policyCategoryDic = state.policyCategoryDic;
    ...
    deleteFilters(event?) {
        const [filters, shouldUpdateComponent] = this.filterManagementService.deleteFilters(
            event,
            this.filters,
        );
                ...
                (row, col, cellObj) => {
                    ...
                    if (col.toLowerCase() === 'policy') {
                        const autoFixAvailable =
                            typeof row.autoFixAvailable == 'string'
                                ? row.autoFixAvailable == 'true'
                                : row.autoFixAvailable;
                        const autoFixEnabled =
                            typeof row.autoFixEnabled == 'string'
                                ? row.autoFixEnabled == 'true'
                                : row.autoFixEnabled;
                        let imgSrc = 'noImg';
                        let imageTitleText = '';
                        if (autoFixAvailable) {
                            imgSrc = autoFixEnabled ? 'autofix' : 'no-autofix';
                            imageTitleText = autoFixEnabled
                                ? 'Autofix Enabled'
                                : 'Autofix Available';
                        }
                        cellObj = {
