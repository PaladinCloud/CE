...
@Component({
    ...
    providers: [IssueListingService, IssueFilterService, LoggerService, ErrorHandlingService],
})
export class VulnerabilitiesComponent implements OnInit, OnDestroy {
            ...
            for (let row = 0; row < getData.length; row++) {
                ...
                for (let col = 0; col < getCols.length; col++) {
                    if (
                        ...
                        getCols[col].toLowerCase() === 'assets affected' ||
                        getCols[col].toLowerCase() === 'assetsaffected'
                    ) {
                        cellObj = {
    ...
    goToDetails(row) {
        ...
        try {
            ...
            if (
                row.col.toLowerCase() === 'assets affected' ||
                row.col.toLowerCase() === 'assetsaffected'
            ) {
                ...
                this.router.navigate(
                    ['../../vulnerabilities/vulnerability-details', row.row.qid.valText],
                    {
                        relativeTo: this.activatedRoute,
