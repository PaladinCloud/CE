...
export class IssueListingComponent implements OnInit, OnDestroy {
    ...
    getUpdatedColumnWidthsAndNamesMap() {
        ...
        this.filterTypeLabels.forEach((label) => {
            if (!excludedColumnNames.has(label)) {
                ...
                const columnName = Object.entries(this.columnNamesMap).find(
                    ([_, value]) => value === label,
                )?.[0];

                if (!columnName) {
                    const apiColName = this.filterTypeOptions.find(
    ...
    preApplyStatusFilter(state) {
        ...
        const statusFilterExists = isStateFiltersArray
            ? this.filters.some((item) => item.keyDisplayValue === 'Status')
            : false;
