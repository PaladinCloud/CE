...
export class RecommendationsDetailsComponent implements OnInit, OnDestroy {
    ...
    getRecommandationsInfoData() {
        ...
        try {
            this.recommandationsInfoSubscription = this.commonResponseService
                .getData(recommendationsInfoUrl, recommendationsInfoMethod, {}, queryParams)
                .subscribe(
                    (response) => {
                        try {
    ...
    getRecommandationsDetailsData() {
        ...
        try {
            if (this.recommendationParams['tags.Application.keyword']) {
                queryParams['filter']['application'] =
                    this.recommendationParams['tags.Application.keyword'];
            } else if (
                this.recommendationParams['filter'] &&
                this.recommendationParams.filter.includes('Application')
            ) {
                const filter = this.utils.processFilterObj(this.recommendationParams);
                queryParams['filter']['application'] = filter['tags.Application.keyword'];
            }
            this.recommandationsInfoSubscription = this.commonResponseService
                .getData(recommendationsdetailsUrl, recommendationsdetailsMethod, queryParams, {})
                .subscribe(
                    (response) => {
                        try {
    ...
    goToDetails(row) {
        try {
            this.workflowService.addRouterSnapshotToLevel(
                this.router.routerState.snapshot.root,
                0,
                this.recommendationParams['name'],
            );
            if (row.col.toLowerCase() === 'resource id') {
                ...
                this.router
                    .navigate(['../../../../../assets/asset-list', resourceType, resourceId], {
