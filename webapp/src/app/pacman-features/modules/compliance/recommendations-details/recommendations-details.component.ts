...
export class RecommendationsDetailsComponent implements OnInit, OnDestroy {
  ...
  getRecommandationsInfoData() {
    ...
    try {
      this.recommandationsInfoSubscription = this.commonResponseService
        .getData(
          recommendationsInfoUrl,
          recommendationsInfoMethod,
          {},
          queryParams
        )
        .subscribe(
  ...
  getRecommandationsDetailsData() {
    ...
    const recommendationsdetailsMethod =
      environment.recommendationsDetails.method;
    const queryParams = {
      ag: this.selectedAssetGroup,
      filter: {
        recommendationId: this.recommendationParams["recommendationId"],
    ...
    try {
      if (this.recommendationParams["tags.Application.keyword"]) {
        ...
        queryParams["filter"]["application"] =
          filter["tags.Application.keyword"];
      }
      this.recommandationsInfoSubscription = this.commonResponseService
        .getData(
          recommendationsdetailsUrl,
          recommendationsdetailsMethod,
          queryParams,
          {}
        )
        .subscribe(
          (response) => {
            try {
              ...
              if (response.data.response.length > 0) {
                ...
                const updatedResponse = this.massageData(
