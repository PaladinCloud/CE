...
export class PolicyViolationsListComponent implements OnInit, OnDestroy {
            ...
            this.issueListingSubscription = this.commonResponseService
                ...
                .subscribe(
                    (response) => {
                        if (
                            !response ||
                            !this.utils.checkIfAPIReturnedDataIsEmpty(response.data.response)
                        ) {
                            // Data is available
                            this.errorValue = 1;
    ...
    getFilterObject(filterArray) {
        ...
        const filterText = this.utils.arrayToObject(this.filterArray, 'filterkey', 'value'); // <-- TO update the queryparam which is passed in the filter of the api
