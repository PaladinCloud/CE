...
export class DetailsInfoComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        for (let i = 0; i < keys.length; i++) {
            let obj = {};
            if (
                keys[i] === 'discoverydate' ||
                keys[i] === '_lastUpdated' ||
                keys[i] === 'firstdiscoveredon' ||
                keys[i] === 'launchtime'
            ) {
    ...
    getVal(val) {
        if (
            (val.values || val.values === 0 || val.values === false) &&
            JSON.stringify(val.values).match('href') &&
            JSON.stringify(val.values).match('http')
        ) {
            val.isLink = true;
