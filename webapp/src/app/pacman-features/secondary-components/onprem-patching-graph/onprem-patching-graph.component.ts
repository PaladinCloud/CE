...
export class OnpremPatchingGraphComponent implements OnInit, OnDestroy {
        ...
        setTimeout(() => {
            ...
            y0.domain([
                ...
                d3Array.max(data, function (d) {
                    return Math.max(d['patched'], d['projected'], 5);
                }),
            ]);
            y1.domain([
                0,
                d3Array.max(newData, function (d) {
                    return Math.max(d['totalPatched'], d['totalProjected'], 5);
                }),
            ]);
    ...
    redirectTo(page) {
        ...
        if (page === 'updateAssets') {
            this.router.navigate(['../../assets/update-assets'], {
                relativeTo: this.activatedRoute,
                queryParamsHandling: 'merge',
            });
        } else if (page === 'patchingProjections') {
            this.router.navigate(['../patching-projections'], {
                relativeTo: this.activatedRoute,
                queryParamsHandling: 'merge',
            });
        }
