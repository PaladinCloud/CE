...
export class AssetInfoComponent implements OnInit, OnDestroy, OnChanges {
                        ...
                        (response) => {
                            try {
                                if (!this.utils.checkIfAPIReturnedDataIsEmpty(response)) {
                                    this.untaggedApplicationData =
                                        response.response[0][this.targetType];
                                }
                                this.showLoader = false;
                                this.seekdata = false;
                                this.dataComing = true;
    ...
    ngOnDestroy() {
        try {
            ...
            if (this.assetDistributionSubscription) {
                this.assetDistributionSubscription.unsubscribe();
            }
            if (this.subscriptionDomain) {
                this.subscriptionDomain.unsubscribe();
            }
            if (this.applicationUntaggedSubscription) {
                this.applicationUntaggedSubscription.unsubscribe();
            }
