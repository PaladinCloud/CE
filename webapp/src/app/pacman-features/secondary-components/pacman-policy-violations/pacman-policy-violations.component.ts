...
export class PacmanPolicyViolationsComponent implements OnInit, OnChanges, OnDestroy {
    ...
    columnsSortFunctionMap = {
        Status: (a, b, isAsc) => {
            ...
            if (statusOrderMap[AStatus] == statusOrderMap[BStatus]) {
                return (
                    (severityOrderMap[ASeverity] < severityOrderMap[BSeverity] ? -1 : 1) *
                    (isAsc ? 1 : -1)
                );
            }
        ...
        try {
            ...
            if (status == 'fail') {
                this.workflowService.navigateTo({
                    ...
                    nextPageTitle: 'Violation Details',
                });
            }
        } catch (error) {
    ...
    getAllPatchingDetails() {
        ...
        const pacmanPolicyViolationsMethod = environment.pacmanPolicyViolations.method;
