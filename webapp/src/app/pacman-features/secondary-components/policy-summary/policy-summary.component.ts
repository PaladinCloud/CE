...
export class PolicySummaryComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.subscriptionToDomain = this.domainObservableService
            .getDomainType()
            .subscribe((domain) => {
        ...
        if (this.ruleID !== undefined) {
            ...
            try {
                this.dataSubscriber = this.commonResponseService
                    .getData(policySummaryUrl, policySummaryMethod, payload, {})
                    .subscribe(
                        (response) => {
                            try {
    ...
    progressDataProcess(data) {
        ...
        if (data.response[0].compliance_percent !== undefined) {
            compliant = {
                value:
                    data.response[0].assetsScanned == 0
                        ? 'NR'
                        : data.response[0].compliance_percent,
                text: 'Compliant',
                link: false,
                styling: {
                    cursor: 'text',
            ...
            this.colors[0] =
                data.response[0].compliance_percent === 100
                    ? this.complianceColors.compliant
                    : data.response[0].compliance_percent >= 50
                      ? this.complianceColors.intermediate
                      : this.complianceColors['non-compliant'];
