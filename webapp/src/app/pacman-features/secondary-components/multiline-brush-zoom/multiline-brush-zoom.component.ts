...
export class MultilineBrushZoomComponent implements OnInit, OnChanges {
    ...
    private drawLine() {
        ...
        for (let i = 0; i < this.graphData.length; i++) {
            ...
            const lineColor =
                this.lineColorsObject[this.graphData[i].key] || this.lineColorsObject[lineKeys[i]];
            this.focus
                .append('path')
    ...
    private checkNumberOfLogAxisTicks() {
        try {
            ...
            const lastVal = parseInt(tickTexts[tickTexts.length - 1][0].innerHTML, 10);
        ...
        if (this.firstMouseMove > 0) {
            ...
            for (let i = 0; i < self.graphLinesData.length; i++) {
                self.focus
                    ...
                    .attr('fill', self.lineColorsObject[self.graphLinesData[numOfLines - i].key])
                    .attr('height', '18px')
                    .attr('width', '92px')
                    .attr('display', 'none')
                    .attr('text-align', 'middle')
                    .style('stroke', self.lineColorsObject[self.graphLinesData[numOfLines - i].key])
                    .attr('x', -110)
                    .attr('y', -7);
        ...
        function mousemove() {
            try {
                ...
                self.legendHover.map(function (legend) {
                    for (let i = 0; i < self.graphLinesData.length; i++) {
                        const currentLineValues = self.graphLinesData[numOfLines - i].values;
