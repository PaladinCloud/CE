...
export class OverviewVulnerabilitiesComponent implements OnInit, OnDestroy {
    ...
    getComplianceCategories(category): void {
        try {
            ...
            const categoryUrl =
                environment.complianceCategories.complianceCategoriesData[category].url;

            const categoryMethod =
                environment.complianceCategories.complianceCategoriesData[category].method;

            const categoryType = category;

            this.subscriptionToOverviewVulnerabilities = this.complianceCategoriesService
                .getData(queryParams, categoryType, categoryUrl, categoryMethod)
                .subscribe(
                    (response) => {
                        this.showError = false;
