...
export class MultilineChartComponent implements OnInit, OnChanges {
    ...
    getIssues() {
        if (this.dataResponse) {
            ...
            try {
                ...
                this.drawLine();
                if (
                    this.graphData[0].values.length >= 2 &&
    ...
    private initSvg() {
        ...
        if (this.axisUnit === 'false') {
            ...
            d3.select(uniqueId)
                .select('svg')
                .attr('width', this.graphWidth - 40);
        }

        d3.select(uniqueId)
            .select('svg')
            .attr('height', this.graphHeight - 59);
    ...
    private initComponents() {
        ...
        const maxValue = d3Array.max(this.graphData, function (c) {
        ...
        if (this.translateChange === true) {
            if (maxValue > 10) {
                ...
                this.y.domain([
                    0,
                    d3Array.max(this.graphData, function (c) {
                        return d3Array.max(c[`values`], function (d) {
                            return d[`value`] * 1.3;
                        });
                    }),
        ...
        if (this.smallerLine !== undefined) {
            for (let i = 0; i < this.smallerLine[`values`].length; i++) {
                ...
                const smallerLineFormattedDate =
                    smallerLineDate.getUTCMonth() +
                    '/' +
                    smallerLineDate.getUTCDate() +
                    '/' +
                    smallerLineDate.getUTCFullYear();
                ...
                for (let j = 0; j < this.longerLine[`values`].length; j++) {
                    ...
                    const longerLineFormattedDate =
                        longerLineDate.getUTCMonth() +
                        '/' +
                        longerLineDate.getUTCDate() +
                        '/' +
                        longerLineDate.getUTCFullYear();
            ...
            this.focus
                ...
                .call(
                    d3Axis
                        .axisLeft(this.y)
                        .ticks(3)
                        .tickFormat((d) => this.abbreviateNumber(d, 'top')),
                );
            ...
            this.focus
                ...
                .call(
                    d3Axis
                        .axisLeft(this.y)
                        .ticks(3)
                        .tickFormat((d) => this.abbreviateNumber(d, 'bottom')),
                );
    ...
    abbreviateNumber(number, value) {
        if (value === 'top') {
            ...
            if (number < '100') {
                ...
                number =
                    number > 1000000
                        ? number / 1000000 + this.axisUnit
                        : number >= 1000
                          ? number / 1000 + this.axisUnit
                          : number >= 100
                            ? number / 1000 + this.axisUnit
                            : number;
                return number;
    ...
    private drawLine() {
        ...
        this.svg.select('#clip rect').transition().duration(2000).attr('width', this.width);
    }
    ...
    private drawHover() {
        ...
        this.focus.append('text').attr('class', 'dateData').attr('x', 9).attr('dy', '.35em');

        this.focus.append('text').attr('class', 'yearData').attr('x', 9).attr('dy', '1.5em');
        ...
        function mousemove() {
            ...
            const getIssuesForHoverDate = self.combinedData.filter(function (issue) {
                const issueDate =
                    issue.x0.getDate() + '-' + issue.x0.getMonth() + '-' + issue.x0.getFullYear();
                const labelDate =
                    dobj[`label`].getDate() +
                    '-' +
                    dobj[`label`].getMonth() +
                    '-' +
                    dobj[`label`].getFullYear();

                return issueDate === labelDate;
            });

            self.legendHover.map(function (legend) {
                const searchedObj = self.searchAnObjectFromArray(
                    'key',
                    legend,
                    getIssuesForHoverDate,
                );
            ...
            for (let k = 0; k < self.dataResponse.length; k++) {
                if (dobj[`value` + k] === 'NO DATA') {
                    self.focus.select('.c' + k).attr('r', 0);
                } else {
                    self.focus
                        .select('.c' + k)
                        .attr(
