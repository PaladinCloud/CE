...
export class PatchingTrendComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    showCurrentQuarter() {
        ...
        let patchingData = this.dataStore.get(
            'patchingAvailableQuarters-' + this.selectedAssetGroup,
        );
    ...
    getMetaData(year?: any, index?: any) {
        try {
            ...
            this.tempLoader = true;
            this.yearArray = [];
            let yearlyQuarterData = this.dataStore.get(
                'patchingAvailableQuarters-' + this.selectedAssetGroup,
            );
            ...
            if (!yearlyQuarterData || yearlyQuarterData.length === 0) {
                ...
                this.getPatchingQuarterData = this.allPatchingProgressService
                    .getQuarterData(payload, patchingQuarterUrl, patchingQuarterMethod, queryParams)
                    .subscribe(
                        (response) => {
                            try {
                                if (response.length) {
    ...
    getIssues(data, year?: any, index?: any) {
        try {
            ...
            const payload = {};
            this.getPatchingSubscriptionQuarter = this.allPatchingProgressService
                .getData(
                    data,
                    patchingQuarterUrl,
                    patchingQuarterMethod,
                ...
                .subscribe(
                    (response) => {
                        ...
                        if (
                            year === undefined ||
                            (index !== undefined && year !== new Date().getFullYear()) ||
