...
export class AllCertificateTableComponent implements OnInit, OnDestroy {
    ...
    getAllPatchingDetails() {
        ...
        this.dataSubscription = this.commonResponseService
            .getData(allcertificateTableUrl, allcertificateTableMethod, payload, {})
            .subscribe(
                (response) => {
                    this.showGenericMessage = false;
                    try {
        ...
        for (let row = 0; row < getData.length; row++) {
            ...
            for (let col = 0; col < getCols.length; col++) {
                if (
                    ...
                    getCols[col].toLowerCase() === 'expiring in'
                ) {
                    cellObj = {
                        link: '',
                        properties: {
                    ...
                    getCols[col].toLowerCase() === 'validfrom' ||
                    getCols[col].toLowerCase() === 'valid from' ||
                    getCols[col].toLowerCase() === 'validuntil' ||
                    getCols[col].toLowerCase() === 'valid until'
                ) {
                    cellObj = {
                        link: '',
                        properties: {
