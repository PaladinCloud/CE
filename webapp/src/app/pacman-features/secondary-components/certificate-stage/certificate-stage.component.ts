...
export class CertificateStageComponent implements OnInit, OnDestroy {
    ...
    getProgressData() {
        ...
        try {
            this.dataSubscriber = this.commonResponseService
                .getData(certificateStageUrl, certificateStageMethod, {}, queryParams)
                .subscribe(
                    (response) => {
                        try {
    ...
    stageDataProcess(data) {
        ...
        localDataKeys.forEach((element, index) => {
            ...
            if (
                (leftBarWidth === 0 && rightBarWidth === 0) ||
                (checkLeftWidth === true && checkRightWidth === true)
            ) {
                leftBarWidth = 1;
                rightBarWidth = 1;
            }
            ...
            if (localData[element].expiry45Days === 0 || localData[element].expiry30Days === 0) {
                if (
                    localData[element].expiry45Days === 0 &&
                    localData[element].expiry30Days !== 0
                ) {
    ...
    navigatePage(event) {
        ...
        try {
            ...
            if (this.routeTo !== undefined) {
                const eachParams = {
                    'tags.Application.keyword': this.capitalizeFirstLetter(currentApp),
                    expiringIn: '45',
                };
