...
export class InstalledSoftwaresComponent implements OnInit, OnDestroy {
    ...
    getAllPatchingDetails() {
        ...
        this.dataSubscription = this.commonResponseService
            .getData(newUrl, method, payload, queryParam)
            .subscribe(
                (response) => {
                    this.showGenericMessage = false;
                    try {
    ...
    handlePopClick(rowText) {
        ...
        try {
            ...
            this.downloadService.animateDownload({ ['Installed Softwares']: true });
            ...
            this.downloadService.animateDownload({ ['Installed Softwares']: false });
