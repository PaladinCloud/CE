...
@Component({
    ...
    providers: [
        MultilineChartServiceCpu,
        MultilineChartServiceDisk,
        MultilineChartServiceNetwork,
        AutorefreshService,
    ],
})
export class UtilizationContainerComponent implements OnInit, OnDestroy {
    ...
    getIssues() {
        ...
        if (this.multilineChartDiscSubscription) {
            this.multilineChartDiscSubscription.unsubscribe();
        }
        if (this.multilineChartCpuSubscription) {
            this.multilineChartCpuSubscription.unsubscribe();
        }
        if (this.multilineChartNetworkSubscription) {
        ...
        this.multilineChartCpuSubscription = this.multilineChartServiceCpu
            .getData(queryParams)
            .subscribe(
                (response) => {
                    try {
        ...
        this.multilineChartDiscSubscription = this.multilineChartServiceDisk
            .getData(queryParams)
            .subscribe(
                (response) => {
                    try {
                        ...
                        if (
                            !this.utilService.checkIfAPIReturnedDataIsEmpty(response[0][0].values)
                        ) {
                            this.showerrorDisk = false;
                            this.diskData = response[0];
                            this.handleUnitDisk(response[0]);
                            this.widgetWidthDisk = parseInt(
                                window
                                    .getComputedStyle(this.widgetContainerDisk.nativeElement, null)
                                    .getPropertyValue('width'),
                                10,
                            );
                            this.widgetHeightDisk = parseInt(
                                window
                                    .getComputedStyle(this.widgetContainerDisk.nativeElement, null)
                                    .getPropertyValue('height'),
                                10,
        ...
        this.multilineChartNetworkSubscription = this.multilineChartServiceNetwork
            .getData(queryParams)
            .subscribe(
                (response) => {
                    try {
                        ...
                        if (
                            ...
                            this.widgetWidthNet = parseInt(
                                window
                                    .getComputedStyle(
                                        this.widgetContainerNetwork.nativeElement,
                                        null,
                                    )
                                    .getPropertyValue('width'),
                                10,
                            );
                            this.widgetHeightNet = parseInt(
                                window
                                    .getComputedStyle(
                                        this.widgetContainerNetwork.nativeElement,
                                        null,
