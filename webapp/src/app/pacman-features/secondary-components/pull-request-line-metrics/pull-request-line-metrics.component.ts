...
export class PullRequestLineMetricsComponent implements OnInit, OnDestroy, OnChanges {
                ...
                (response) => {
                    try {
                        ...
                        if (this.dataResponse.pullRequestStateByWeek.length === 0) {
                            ...
                            this.computeYearObj();
                            this.dataState = 1;
                            this.selectedWeek = this.dataResponse.pullRequestStateByWeek.length - 1;
                            for (
    ...
    loadNewData() {
        try {
            ...
            this.maxVal = [0, 0];
            this.dataResponse =
                this.finalData[this.selectedYear].quartersData[this.selectedQuarter].data;
            this.selectedWeek =
                this.finalData[this.selectedYear].quartersData[this.selectedQuarter].rawData
                    .pullRequestStateByWeek.length - 1;
            this.processData(this.dataResponse.pullRequestStateByWeek);
            this.plotDonut(this.selectedWeek);
        } catch (e) {
            this.logger.log('error', e);
    ...
    computeYearObj() {
        try {
            ...
            this.finalData[this.selectedYear].quartersData[
                this.finalData[this.selectedYear].quartersData.length - 1
        ...
        setTimeout(() => {
            ...
            y0.domain([
                d3Array.min(newData, function (d) {
                    return Math.min(d['totalCreated'], d['totalMerged'], d['totalDeclined'], 5);
                }),
                d3Array.max(newData, function (d) {
                    return Math.max(d['totalCreated'], d['totalMerged'], d['totalDeclined'], 5);
                }),
            ]);
    ...
    findMaxVal(obj) {
        try {
            if (!isNaN(obj.totalMerged) && !isNaN(obj.totalCreated) && !isNaN(obj.totalDeclined)) {
                this.maxVal[0] = Math.max(this.maxVal[0], obj['totalMerged'], obj['totalCreated']);
                for (let i = 5; i > 0; i--) {
    ...
    processBarData(data) {
        ...
        const result = pick(data, filter.split(', '));
    ...
    checkIfEmptyObj() {
        try {
            for (let i = 0; i < this.finalData[this.yearPickedIndex].quartersData.length; i++) {
                if (
                    Object.keys(this.finalData[this.yearPickedIndex].quartersData[i].rawData)
                        .length === 0
                ) {
                    this.callNewData(
                        this.finalData[this.yearPickedIndex].year,
                        this.finalData[this.yearPickedIndex].quartersData[i].number,
                        i,
                    );
                }
            }
            ...
            .subscribe(
                (response) => {
                    try {
                        ...
                        this.finalData[this.currentYear - year].quartersData[quarterIndex].loading =
                            false;
                        if (dataResponse.pullRequestStateByWeek.length === 0) {
                            this.finalData[this.currentYear - year].quartersData[
                                quarterIndex
                            ].errorOccured = "Oops! It's deserted here.";
                        } else {
                            this.finalData[this.currentYear - year].quartersData[
                                quarterIndex
                        ...
                        this.finalData[this.currentYear - year].quartersData[
                            quarterIndex
                        ].errorOccured = 'Oops! Something went wrong while fetching your data.';
                    }
                },
                (error) => {
                    this.logger.log('error', error);
