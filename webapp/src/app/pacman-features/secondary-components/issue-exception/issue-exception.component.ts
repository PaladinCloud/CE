...
export class IssueExceptionComponent implements OnInit, OnDestroy {
    ...
    getData() {
        if (this.issueList.length > 0) {
            ...
            for (let i = 0; i < this.issueList.length; i++) {
                this.totalIssues.push(this.issueList[i]['Violation ID'].text);
                if (this.issueList[i]['Status'].text === 'open') {
                    this.openIssueList.push(this.issueList[i]['Violation ID'].text);
                } else {
                    this.exemptedIssueList.push(this.issueList[i]['Violation ID'].text);
                }
            }
        }
    }
    ...
    verifyResponse(data) {
        if (data.status === 'Success' && data.failedIssueIds.length === 0) {
            ...
            (data.status === 'Partial Success' || data.status === 'Failed') &&
            data.failedIssueIds.length > 0
        ) {
            this.actionComplete = false;
    ...
    copytext() {
        ...
        this.toastObservableService.postMessage('Issue IDs copied to clipboard');
    }
