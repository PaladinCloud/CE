...
export class TaggingInstancesTableComponent implements OnInit, OnDestroy {
    ...
    getTaggingSummary() {
        ...
        this.dataSubscription = this.commonResponseService
            .getData(taggingSummaryUrl, taggingSummaryMethod, payload, {})
            .subscribe(
                (response) => {
                    this.showGenericMessage = false;
                    try {
    ...
    processData(data) {
        ...
        for (let row = 0; row < getData.length; row++) {
            ...
            for (let col = 0; col < getCols.length; col++) {
                if (
                    (getCols[col].toLowerCase() === 'environment untagged' ||
                        getCols[col].toLowerCase() === 'role untagged' ||
                        getCols[col].toLowerCase() === 'stack untagged') &&
                    getData[row][getCols[col]] > 0
                ) {
                    cellObj = {
                        link: 'true',
                        properties: {
    ...
    goToDetails(row) {
        try {
            ...
            if (
                row.col.toLowerCase() === 'environment untagged' &&
                row.row['environment Untagged'].text > 0
            ) {
                ...
                row.col.toLowerCase() === 'role untagged' &&
                row.row['role Untagged'].text > 0
            ) {
                ...
                row.col.toLowerCase() === 'stack untagged' &&
                row.row['stack Untagged'].text > 0
            ) {
