...
export class PatchingCurrentStateComponent implements OnInit, OnDestroy {
    ...
    getPatchingCurrentState() {
        ...
        this.dataSubscription = this.commonResponseService
            .getData(patchingStateUrl, patchingStateMethod, {}, queryParams)
            .subscribe(
                (response) => {
                    try {
    ...
    getPatchingExecState() {
        ...
        this.dataSubscriptionExec = this.commonResponseService
            .getData(patchingExecUrl, patchingExecMethod, {}, queryParams)
            .subscribe(
                (response) => {
                    try {
                        if (response.response.length === 0) {
