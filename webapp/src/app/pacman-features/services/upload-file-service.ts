...
export class UploadFileService {
    ...
    pushFileToStorage(url, method, file: File, formDataValues) {
        ...
        try {
            return this.httpService.getHttpResponse(url, method, formdata).pipe(
                map((response) => {
    ...
    postData(url, payload, headers) {
        let httpObservable = this.http.post(url, payload, headers).pipe(
            map((response) => {
    ...
    convertQueryParametersToString(queryParams: any) {
        ...
        if (!this.utilityService.isObjectEmpty(queryParams)) {
            ...
            Object.keys(queryParams).forEach((param) => {
                queryParamString +=
                    queryParams[param] !== '' && queryParams[param] !== undefined
    ...
    getData(url, headers) {
        let httpObservable = this.http.get(url, headers).pipe(
            map((response) => {
