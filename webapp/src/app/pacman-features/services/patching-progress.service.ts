    ...
    getData(data, url, method, assetGroup, year?: any): Observable<any> {
        try {
            if (method !== 'GET') {
                ...
                this.year = 2018;
                try {
                    return this.httpService.getHttpResponse(url, method, {}, {}).pipe(
                        map((response) => {
    ...
    massageDataYearly(quarters, data): any {
        if (quarters === 'null') {
          ...
          then fill up the extra dates with -1 */
            ...
            if (
                end_dateQuarter !==
                response.patching_progress[length - 1].patching_info[newLength - 1].date
            ) {
                this.lastDate =
                    response.patching_progress[length - 1].patching_info[newLength - 1].date;
                if (response.patching_progress[length - 1].patching_info.length < 7) {
                    ...
                    for (
                        let s = 1;
                        s <= 7 - response.patching_progress[length - 1].patching_info.length;
                        s++
                    ) {
                        const extraDays = tomorrow.setTime(
                            lastDay.getTime() + 24 * 60 * 60 * 1000 * s,
                        );
                    ...
                    response.patching_progress[length - 1].patching_info.push.apply(
                        response.patching_progress[length - 1].patching_info,
                        additionalWeeks,
                    );
                }
            }
          ...
          then fill up the rest of the weeks with -1 */
            if (length < numberOfWeeks) {
                ...
                for (let q = 0; q < remainingWeeks; q++) {
                    ...
                    for (let t = 1; t <= 7; t++) {
                        const nextDay = tomorrow.setTime(
                            this.endDate.getTime() + 24 * 60 * 60 * 1000 * t,
                        );
                        restDays = {
                            patched_instances: -1,
                            total_instances: -1,
                ...
                for (let i = 0; i < lastWeekResponse.length; i++) {
                    if (
                        new Date(lastWeekResponse[i].date).getMonth() ===
                        new Date(response.end_date).getMonth()
                    ) {
                        if (
                            new Date(lastWeekResponse[i].date).getDate() ===
                            new Date(response.end_date).getDate()
                        ) {
                            indexValue = i;
                        }
                    }
            ...
            if (response.projection_info !== undefined) {
                if (response.projection_info.length > 0) {
                    for (let i = 0; i < response.projection_info.length; i++) {
                        const firstDateValue = new Date(
                            tomorrowDate.setTime(this.firstDate.getTime() + 86400000 * (7 * i)),
                        );
                        for (let j = 0; j < 7; j++) {
                            objProjection = {
                                date: new Date(
                                    tomorrowDate.setTime(firstDateValue.getTime() + 86400000 * j),
                                ),
