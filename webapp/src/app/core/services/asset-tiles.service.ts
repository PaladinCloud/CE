...
export class AssetTilesService {
    ...
    getAssetTiles(assetUrl, assetMethod): Observable<any> {
        ...
        try {
            return combineLatest(
                this.httpService
                    .getHttpResponse(url, method, payload)
                    .pipe(map((response) => this.massageData(response))),
            );
        } catch (error) {
            this.errorHandlingService.handleJavascriptError(error);
                ...
                try {
                    ...
                    if (!assetGroupListSessionStored) {
                        this.getAssetGroupList().subscribe((response) => {
                            const assetGroupObject = this.filterAssetGroupObjectFromList(
                                assetGroupName,
                                response,
                            );
                            const assetGroupDisplayName = assetGroupObject
                                ? assetGroupObject.displayname
                                : assetGroupName;
    ...
    filterAssetGroupObjectFromList(assetGroupName, assetGroupList) {
        try {
            ...
            const filteredArray = assetGroupList.filter(
    ...
    getListOfDomainsAssociatedWithAssetGroup(assetGroupName) {
        try {
            return new Observable((observer) => {
                let listOfDomains = this.dataStore.getCurrentSelectedDomainList()
                    ? this.dataStore.getCurrentSelectedDomainList()
                    : '';
                if (listOfDomains) {
