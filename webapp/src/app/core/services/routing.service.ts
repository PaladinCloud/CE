...
export class RoutingService {
    ...
    redirectToRespectiveLatestPath(moduleName, agAndDomain) {
        try {
            ...
            if (savedPath) {
                const queryParams = this.workflowService.getPreviouslyOpenedPageQueryParamsInModule(
                    moduleName,
                )
                    ? JSON.parse(
                          this.workflowService.getPreviouslyOpenedPageQueryParamsInModule(
                              moduleName,
                          ),
                      )
                    : {};
                ...
                const listOfContextualMenuItems =
