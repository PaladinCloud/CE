...
export class DropdownComponent implements OnChanges {
    ...
    onOpen() {
        if (this.selectAll.length > 0)
            this.selectedAll.checked =
                this.selectedOption.length == this.optionList.length ? true : false;
    ...
    massageData(list: any, selectedOption: any) {
        ...
        if (this.optionImage) {
            if (this.placeholder == 'Category') {
                ...
                this.selectedOptionImage =
                    '../../../../assets/icons/' + this.selectedOption.toLowerCase() + '-color.svg';
            }
        }
    }
    ...
    capitalizeFirstLetter(option: string | unknown[]) {
        if (this.showOriginalText) return option;
