...
export class DataTableComponent implements OnInit, OnChanges {
  ...
  @Output() showExceptionModalEventEmitter = new EventEmitter<string>();
  @Output() paginatorSizeSelectedEventEmitter = new EventEmitter<number>();
  @Output() removeAllCheckedEventEmitter = new EventEmitter<any>();
    ...
    if (this.outerArr !== undefined && this.outerArr.length !== 0) {
      ...
      if (this.tabFilterProperty) {
        ...
        for (let index = 0; index < this.tabsData.length; index++) {
          this.tabsData[index]["displayName"] =
            this.refactorFieldsService.getDisplayNameForAKey(
              this.tabsData[index]["text"].toLowerCase()
            ) || this.tabsData[index]["text"];
        }
      }
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (errorValueChange) {
      if (
        errorValueChange &&
        (errorValueChange.currentValue === 0 ||
          this.errorValue.currentValue === -1)
      ) {
  ...
  showExceptionModal(exception) {
    this.showExceptionModalEventEmitter.emit(exception);
  }

  handlePaginatorSizeSelection(pageSize) {
    this.paginatorSizeSelectedEventEmitter.emit(pageSize);
  }
    ...
    try {
      setTimeout(() => {
        const a = document.getElementsByClassName(
          "data-table-inner-content data-table-current"
        );
        const b = document.getElementsByClassName(
          "data-table-inner-wrap relative"
        );
  ...
  removeAllChecked() {
    this.removeAllCheckedEventEmitter.emit(true);
  }
