...
export class UtilsService {
        ...
        data.forEach((row) => {
            ...
            keysToBeChanged.forEach((element) => {
                ...
                if (columnNamesMap[element]) {
                    ...
                    elementNew =
                        refactoredService.getDisplayNameForAKey(element.toLowerCase()) || element;
                }
    ...
    getColumnNamesMapAndColumnWidthsMap(
        filterTypeLabels,
        filterTypeOptions,
        ...
        filterTypeLabels.forEach((label) => {
            if (!excludedColumnNames.has(label)) {
                ...
                if (!Object.values(columnNamesMap).includes(label)) {
                    const apiColName = filterTypeOptions.find(
                        (option) => option.optionName === label,
                    )?.optionValue;
    ...
    isDateStringValid(dateString) {
        ...
        return (
            datePattern1.test(dateString) ||
            datePattern2.test(dateString) ||
            datePattern3.test(dateString)
        );
    }
    ...
    */
    ...
    findValueInArray(array, valueToBeFound) {
        return array.findIndex((eachValue) => {
            return (
                ...
                (valueToBeFound && valueToBeFound.toLowerCase())
            );
        });
    }
    ...
    mapToObject(map: Map<string, string> | undefined): { [key: string]: string } | undefined {
        return map ? Object.fromEntries(map) : undefined;
    }
