...
export class FilterManagementService {
    ...
    getValuesForFilterType(currentFilterType, queryParam = {}, payload = {}) {
        const url =
            environment.base + this.utils.getParamsFromUrlSnippet(currentFilterType.optionURL).url;
        const method = 'GET';

        try {
            return combineLatest(
                this.httpService
                    .getHttpResponse(url, method, payload, queryParam)
    ...
    getFilterArray(pageLevelAppliedFilters) {
        try {
            ...
            const formattedFilters = dataArray.map((data) => {
                data.name =
                    this.refactorFieldsService.getDisplayNameForAKey(data.name) || data.name;
                return data;
            });
    ...
    getFilters(filterId: number): Observable<IFilterOption[]> {
        return this.issueFilterService
            .getFilters({ filterId }, environment.issueFilter.url, environment.issueFilter.method)
    ...
    async getFilterTagsData(payload, optionUrl) {
        return this.issueFilterService
            .getFilters(
                {},
                environment.base + this.utils.getParamsFromUrlSnippet(optionUrl).url,
                'POST',
                payload,
            )
            .toPromise()
    ...
    getFormattedFilters(filterText, filterTypeOptions) {
        return Object.keys(filterText).map((filterKey) => {
            const keyDisplayValue = filterTypeOptions.find(
                (option) => option.optionValue === filterKey,
            )?.optionName;
            return {
                keyDisplayValue,
                filterkey: filterKey,
        ...
        try {
            if (
                (event &&
                    ...
                    filters[event.index].filterValue == undefined) ||
                !event.clearAll
            ) {
        ...
        try {
            if (currentFilterType) {
                const filterTags = event.filterValue.flatMap((value) => {
                    const v = filterTagOptions[event.filterKeyDisplayValue].filter(
                        (availableOption) => availableOption.name === value,
                    );
                ...
                this.utils.addOrReplaceElement(
                    ...
                    (el) => {
                        return el.compareKey === currentFilterType.optionValue.toLowerCase().trim();
                    },
                );
            }
            const index = filters.findIndex(
                (filter) => filter.keyDisplayValue === currentFilterType.optionName,
            );
    ...
    async changeFilterType({
        ...
        filtersToBePassed = Object.keys(filtersToBePassed).reduce((result, key) => {
            ...
            if (
                !excludedKeys.includes(normalizedKey) &&
                !excludedKeysInUrl.includes(normalizedKey)
            ) {
        ...
        let payload: IFilterPayload = {
            attributeName: ignoreAttributeName
                ? undefined
                : currentFilterType['optionValue']?.replace('.keyword', ''),
        ...
        if (
            !labelsToExcludeSort
                ?.toString()
                .toLowerCase()
                .includes(currentFilterType.optionName.toLowerCase())
        ) {
    ...
    createFilterObj(
        ...
        filterKey: string,
        validFilterValues: { name: string; id: string }[],
    ): IFilterObj {
        return {
