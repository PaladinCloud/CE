    ...
    canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
        ...
        if (!this.authService.authenticated) {
            this.loggerService.log(
                'info',
                'AuthGuard - Authentication required to access this page',
            );
        ...
        this.loggerService.log(
            'info',
            'AuthGuard - User authenticated, user is granted access for this page',
        );
        const checkUserRoles = this.dataStore.getUserDetailsValue().getRoles();
        if (checkUserRoles.length <= 0) {
            this.authService.setUserFetchedInformation().subscribe(
                (response) => {
        ...
        if (
            state.url.includes('vulnerabilities-compliance') &&
            !CONFIGURATIONS.optional.general.qualysEnabled
        ) {
