...
export class MainFilterComponent implements OnInit {
    ...
    onloadSetValues() {
        ...
        if (
            ...
            if (
                ...
                this.secondaryLevelData = this.filterData['groupBy'].values[this.firstLevelIndex];
            }
            if (
                !(
                ...
                const thirdLevelData =
                    this.filterData['groupBy'].values[0]['groupBy'].values[this.secondLevelIndex];
                thirdLevelData.groupBy.values.forEach((element) => {
                    element.displayName =
                        this.refactorFieldService.getDisplayNameForAKey(
    ...
    toggleFilter() {
        this.hideFilter === false ? (this.hideFilter = true) : (this.hideFilter = false);
        this.filterOpenEvent.emit(this.hideFilter);
    }
        ...
        try {
            ...
            for (const i in obj) {
                ...
                if (typeof obj[i] === 'object') {
                    objects = objects.concat(this.findAndReplace(obj[i], key, val, newValue));
    ...
    storeFirstLevel(data, index, event?) {
        try {
            ...
            this.filterQuery = this.findAndReplace(this.filterQuery, 'applied', true, false);
            ...
            this.findAndReplace(
                this.filterQuery['groupBy'].values[this.firstLevelIndex]['groupBy'].values,
                'applied',
                true,
                false,
            );
    ...
    storeSecondLevel(data, index) {
        try {
            data.groupBy.values.forEach((element) => {
                element.displayName =
                    this.refactorFieldService.getDisplayNameForAKey(element.name.toLowerCase()) ||
                    element.name;
            });
    ...
    storeThirdLevel(data, index1, index2, checkBoxChecked) {
        try {
            ...
            this.filterQuery['groupBy'].values[this.firstLevelIndex]['groupBy'].values[
                this.secondLevelIndex
            ]['groupBy'].values[this.thirdLevelIndex].values[this.fourthLevelIndex].applied =
                checkBoxChecked;
            this.filterOptionClicked.emit(this.filterQuery);
            /**
            ...
            this.numOfCheckedCount(
                this.filterQuery['groupBy'].values[this.firstLevelIndex]['groupBy'].values[
                    this.secondLevelIndex
                ]['groupBy'].values[this.thirdLevelIndex].values,
                index1,
            );
            this.dataStore.set(
