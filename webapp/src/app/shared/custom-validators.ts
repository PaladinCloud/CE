...
import {
    ...
    LOWERCASE_ALPHANUMERIC_HYPHEN,
    NOT_SPECIAL_CHARACTERS,
    ONLY_ALPHABETS,
    ONLY_ALPHANUMERIC,
    STARTS_WITH_ALPHABET,
    STARTS_WITH_ALPHABET_ALLOWS_ANY,
    URL_PATTERN,
...
export class CustomValidators extends Validators {
    static validateCharacters(control: FormControl) {
        if (control.value && control.value.length > 0) {
            ...
            return matches && matches.length ? { not_allowed_characters: matches } : null;
        } else {
            return null;
        }
    }
    ...
    static noStartingNumberOrSpecialCharacter(
        ...
        return STARTS_WITH_ALPHABET_ALLOWS_ANY.test(value)
            ? null
            : { noStartingNumberOrSpecialCharacter: true };
    }
