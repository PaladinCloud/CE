...
export class GenericPageFilterComponent implements OnInit, OnDestroy, OnChanges {
        ...
        try {
            ...
            if (this.filterId) {
                this.filterSubscription = this.filterManagementService
                    .getApplicableFilters(this.filterId)
                    .subscribe(
                        (response) => {
    ...
    changeFilterType(value) {
        try {
            ...
            this.disableFilterTags = false;
            this.filterValues.currentSelectedFilterType = find(
                this.filterValues.filterTypeOptions,
                {
            ...
            this.filterSubscription = this.filterManagementService
                .getValuesForFilterType(this.filterValues.currentSelectedFilterType, queryParams)
                .subscribe(
                    (response) => {
                        this.dataAvailable = true;
                        ...
                        if (
                            this.filterValues.filterValuesOptions.length === 1 &&
                            this.selectOnSingleValue
                        ) {
    ...
    changeFilterValue(value, mandatory?) {
        try {
            if (this.filterValues.currentSelectedFilterType) {
                const filterTag = mandatory
                    ? value
                    : find(this.filterValues.filterValuesOptions, { name: value.value });
                this.utils.addOrReplaceElement(
                    this.filterArray,
                    {
                        ...
                        value: filterTag.id,
                        filterkey:
                            this.filterValues.currentSelectedFilterType['optionValue'].trim(),
                        compareKey: this.filterValues.currentSelectedFilterType['optionValue']
                            .toLowerCase()
                            .trim(),
                    },
                    (el) => {
                        return (
                            el.compareKey ===
                            this.filterValues.currentSelectedFilterType['optionValue']
                                .toLowerCase()
                                .trim()
                        );
                    },
            ...
            if (mandatory) {
                ...
                this.filterArray['mandatory'] =
                    this.filterValues['filterTypeOptions'][0].optionValue;
            }
