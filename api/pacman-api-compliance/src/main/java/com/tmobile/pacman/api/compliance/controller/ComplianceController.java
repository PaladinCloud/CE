...
public class ComplianceController implements Constants {
    ...
    @ApiOperation(httpMethod = "GET", value = "API to get compliance details(number of passed checks, total checks) category wise for the asset group.", produces = MediaType.APPLICATION_JSON_VALUE)
    @RequestMapping(path = "/v1/compdatabycategory", method = RequestMethod.GET)
    public ResponseEntity<Object> getComplianceByCategoryAndAssetGroup(@ApiParam(value = "Asset group", required = true)
                                                                       @RequestParam("ag") String assetGroup) {
        try{
            return ResponseUtils.buildSucessResponse(complianceService.getCategoryCompDetails(assetGroup));
        } catch (Exception exception) {
            log.error("Unexpected error occurred!!", exception);
            return ResponseUtils.buildFailureResponse(new Exception("Unexpected exception occurred inside ComplianceController.getComplianceByCategoryAndAssetGroup!!"), exception.getMessage());
        }
    }

    @PostMapping(path = "/v1/policy-compliance-by-category")
    public ResponseEntity<Object> getPolicyComplianceOverviewAsList(@RequestBody(required = true) Request request) {
        String assetGroup = request.getAg();
        Map<String, String> filters = request.getFilter();
        if (Strings.isNullOrEmpty(assetGroup) || filters == null || Strings.isNullOrEmpty(filters.get(DOMAIN))) {
            return ResponseUtils.buildFailureResponse(new Exception(ASSET_GROUP_DOMAIN));
        }
        ResponseWithOrder response = null;
        try {
            response = (complianceService.getPolicyComplianceOverviewAsList(request));
        } catch (Exception e) {
            return ResponseUtils.buildFailureResponse(new Exception("Unexpected exception occurred inside ComplianceController.getPolicyComplianceOverviewAsList!!"), e.getMessage());
        }
        return ResponseUtils.buildSucessResponse(response);
